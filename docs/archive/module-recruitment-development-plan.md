...
> ✅ **Seluruh Gap Analysis (G-1 s.d. G-12) SELESAI per 2026-08-13.** Modul Recruitment & Onboarding kini operasional penuh — backend (33 gap item lintas G-1 s.d. G-11) dan frontend (G-12, 3 sub-project) sama-sama tuntas. Item di luar scope tetap: strategic layer (§5.2, dikelola plan terpisah) dan beberapa keterbatasan yang didokumentasikan secara sengaja (Time to Fill/Time to Stage analytics §G-11; interview scheduling day-level bukan datetime, assessment participation N+1 client-side §G-12).
> ✅ **G-11 selesai (2026-08-13):** recruitment analytics (tanpa migration baru — `GET /recruitment/analytics/summary?from=&to=` menghitung counts + time-to-hire + offer-acceptance-rate + application-conversion-rate + source-conversion on-the-fly; Time to Fill/Time to Stage/match-score-aggregate sengaja tidak diimplementasikan — didokumentasikan sebagai gap, lihat §G-11) — lihat §G-11.
> ✅ **G-12 selesai (2026-08-13, 3 sub-project):** sub-1 Candidates (list + profile 6-tab G-6, Create+List+Delete per tab); sub-2 summary cards di hub (fail-silent); sub-3 Applications+Screening+Assessment+Interviews **digabung jadi satu halaman bertab** (`Applications.vue` + `ApplicationDetail.vue`) alih-alih 3 halaman terpisah + kanban board seperti rencana asli — lihat §G-12 untuk detail keputusan scoping.
...
- **Integrated Recruitment (approval, offer, stage history, screening, assessment, scorecard, candidate enhancement, integrasi operasional, FE)** — ✅ **selesai**: **G-1** approval requisition + **G-2** requisition enhancement + **G-3** offer management + **G-4** Recruitment → Employee/Movement + **G-5** stage history + **G-6** candidate enhancement + **G-7** screening & assessment + **G-8** interview scorecard + **G-9** candidate matching + **G-10** onboarding scoping + **G-11** analytics + **G-12** frontend (Candidates, summary cards, Applications+Screening+Assessment+Interviews) — seluruhnya tuntas 2026-08-12/13 (lihat Gap Analysis §7 untuk detail per item).
...
## 3.2 Frontend — ✅ SELESAI (seluruh halaman operasional G-12 sudah ada)
...
**Yang diimplementasikan:**
- **Tanpa migration baru** — murni agregasi read-only atas data operasional yang sudah ada.
- **Endpoint:** `GET /recruitment/analytics/summary?from=&to=` (query param `from`/`to` unix nano, opsional, membatasi `job_applications.applied_at`).
- **Counts:** `open_requisitions, candidates, applications, shortlisted, interviews, offers, hires, rejected, withdrawn` — dihitung dari `job_requisitions.status=OPEN`, `candidates`, `job_applications` (per status, dalam rentang `from`/`to` bila diisi), `interviews`, `job_offers`.
- **Metrik turunan:**
  - `time_to_hire_days` — rata-rata `(accepted_at - applied_at)` dalam hari atas application ACCEPTED dengan kedua timestamp terisi; `nil` bila tidak ada data.
  - `offer_acceptance_rate` — `ACCEPTED / (ACCEPTED + REJECTED)` offer; `nil` bila tidak ada offer ACCEPTED/REJECTED.
  - `application_conversion_rate` — `ACCEPTED application / total application`; `nil` bila tidak ada application.
  - `source_conversion` — per `candidates.source` (free-text existing): jumlah application + hire + conversion rate, di-join lewat `candidates.id = job_applications.candidate_id`.
- **Sengaja TIDAK diimplementasikan (didokumentasikan sebagai gap, bukan dilupakan):**
  - **Time to Fill** — butuh `job_requisitions.closed_at`, field yang ADA di skema tapi **tidak pernah di-set di manapun** di codebase (dead field sejak `015_recruitment.sql`); angka akurat butuh endpoint requisition-close yang mengisi `closed_at` — di luar scope G-11.
  - **Time to Stage** — butuh agregasi `job_application_stage_histories` (G-5) per stage, dianggap di luar scope minimal G-11 saat brainstorming.
  - **Candidate Match Score aggregate** — skor per-application sudah ada (G-9 sub-2, `GET /applications/:id/match-score`); mengagregasi ke ringkasan analytics tidak diminta.
- **Service:** `GetRecruitmentAnalyticsSummary(ctx, from, to *int64)` — beberapa query `COUNT`/agregasi Go-side (bukan `AVG` SQL, untuk portabilitas mysql/postgres/sqlite-test) atas `ListApplicationsForAnalytics` + `ListApplicationSourcesForAnalytics` (JOIN candidates).
- **Test:** +6 (service +4: counts end-to-end, time-to-hire + offer-acceptance-rate, empty-data-no-divide-by-zero, date-range-filter; handler +2: summary call, summary dengan query param). Total recruitment: **264** (handler 57 + repository 62 + service 145).

**Ref:** plan asli §37.

## G-12 ✅ FRONTEND RECRUITMENT — sub-project 1 (Candidates) ✅ + sub-project 2 (summary cards) ✅ + sub-project 3 (Applications/Screening/Assessment/Interviews) ✅

**Status: ✅ Selesai (2026-08-13).** Semua halaman FE recruitment operasional sudah ada. **Keputusan scoping penting (brainstorming sub-project 3):** Applications/Screening/Assessment/Interviews **digabung jadi satu halaman** (`Applications.vue` + `ApplicationDetail.vue` bertab) alih-alih 3 halaman terpisah seperti rencana asli — keempatnya berpusat di `job_applications` sehingga natural sebagai satu detail page bertab. **Kanban drag-drop board TIDAK dibuat** (kompleksitas tinggi/ROI rendah dibanding dropdown ubah status) — perubahan status memakai dropdown yang memanggil endpoint transition yang sama persis.
...
- [x] **Candidates (G-12 sub-project 1, 2026-08-13)**: `Candidates.vue` (list + search debounced + create/edit dialog + delete) + `CandidateDetail.vue` (profile: header info + edit + 6 tab G-6 — Educations, Work Experiences, Skills, Certifications, Documents, Consents). **Scope sengaja disederhanakan** (keputusan brainstorming): setiap sub-tab hanya **Create + List + Delete** (tanpa edit in-place — hapus lalu buat ulang bila salah input), konsisten dengan Consents yang backend-nya memang append-only (create+list saja). Skill dropdown memakai master `competency.competencies` existing; education/education_major master **tidak dipakai** — field `major` tetap free-text (fallback yang memang didukung backend G-6). Route `/recruitment/candidates` (list) + `/recruitment/candidates/:id` (detail). Hub card `Candidates` dipindah dari "Coming soon" ke menu aktif.
- [x] **Summary cards di hub (G-12 sub-project 2, 2026-08-13)**: `Recruitment.vue` fetch `GET /recruitment/analytics/summary` (G-11) saat mount, render kartu Open Requisitions/Candidates/Applications/Interviews/Offers/Hires + Time to Hire (bila tersedia) di atas menu grid. **Fail-silent** — kartu tidak tampil sama sekali kalau endpoint error (bukan error state mengganggu), hub tetap fungsional.
- [x] **Applications + Screening + Assessment + Interviews (G-12 sub-project 3, 2026-08-13)**:
  - `Applications.vue` — list (filter status, search via requisition/candidate name resolved client-side) + create dialog (requisition + candidate dropdown).
  - `ApplicationDetail.vue` — header (candidate/requisition/status + dropdown ubah status → `PUT /applications/:id/status`) + 5 tab:
    - **History** — baca-saja, `GET /applications/:id/history` (G-5).
    - **Screening** — Create+List+Delete `application_screenings` (G-7 sub-1); field `screened_by`/`screened_at` presisi tidak diinput manual (disederhanakan — hanya result/score/notes).
    - **Assessment** — "Add to Assessment" (join sesi assessment existing via dropdown) + list partisipasi. **Keterbatasan disengaja:** tidak ada endpoint backend "list partisipasi per application", jadi FE fetch semua assessment lalu N+1 fetch participants per assessment dan filter di client — dapat diterima untuk jumlah sesi assessment yang wajar (puluhan), didokumentasikan sebagai known trade-off bila datanya membesar.
...
- [x] **Requirements & Competencies FE (G-12 susulan, 2026-08-13; Job Management fallback 2026-08-14; diubah dari dialog ke halaman penuh 2026-08-14)**: sebelumnya backend G-9 sub-1 (`job_requisition_requirements`/`job_requisition_competencies`) sudah lengkap sejak commit `b5051d3` tapi **tidak ada FE-nya sama sekali** — celah yang tidak eksplisit tercatat saat brainstorming G-12 awal, ditemukan & ditambal belakangan. Awalnya berbentuk dialog dari `Requisitions.vue`, **diubah jadi halaman penuh** `RequisitionRequirements.vue` (`/recruitment/requisitions/:id/requirements`) atas permintaan user. Kolom aksinya di `Requisitions.vue` juga dipecah: tombol "Requirements" sekarang punya kolom + header sendiri (sebelumnya digabung tanpa label jelas di kolom "Actions" bersama edit/delete/submit). Isi halaman tetap sama: 2 section side-by-side — Requirements (add: requirement_type free-text, name, min/max value; list+delete) dan Competencies (add: dropdown competency dari master `competency.competencies`, required_level, weight; list+delete), keduanya fallback ke Job Management (read-only, border dashed amber) kalau requisition belum punya override sendiri (G-9 sub-project 3).
...
GET    /api/v1/tenant/recruitment/analytics/summary
...
GET    /recruitment/analytics/*                     ← G-11 ✅ (implemented as /analytics/summary, see §8.1)
```
...
- ✅ **Sudah ada**: `015_recruitment.sql` (mysql + postgres) — 7 tabel dasar; `093`-`106` (G-1 s.d. G-11, lihat §7). Tidak ada migration tersisa di rencana — sisa gap (G-12 FE) tidak membutuhkan skema baru.
