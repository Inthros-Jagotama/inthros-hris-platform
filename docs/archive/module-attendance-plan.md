> ✅ **Diarsipkan (2026-08-18)**: modul Attendance dinyatakan selesai oleh user dan dokumen ini dipindahkan ke `docs/archive/`. Catatan jujur: beberapa item di plan awal **masih belum diimplementasikan** pada saat pengarsipan — Absent/Exempt auto-detection (butuh scheduled job §44-45), Manager/HR Dashboard & Team Calendar mandiri (§36/38/39, saat ini hanya terwakili sebagian oleh `HRAttendanceLeaveDashboard.vue`), Payroll integration untuk attendance base (Phase 13), penerapan otomatis `WRONG_CHECKIN`/`WRONG_CHECKOUT` ke session, serta tabel `attendance_device_captures`/`attendance_face_captures` yang tidak punya repository/handler sama sekali. Diarsipkan sebagai keputusan produk (scope dianggap cukup), bukan karena seluruh gap di atas sudah tertutup — lihat riwayat percakapan 2026-08-18 untuk konteks.
>
