...
| Phase 3 - Push-Based Status Sync | ✅ Completed | 2026-08-07 | `c7982fc` — RegisterStatusHandler/notifyStatusChange callback registry, wired ke SubmitAction/CancelInstance/CreateInstance, retrofit payroll (approval_instance_id column, migration 060, HandleApprovalStatusChange, NewModuleWithService), 5 test baru |
| Phase 4 - Migrasi 4 Module Ad-hoc | ✅ Completed | 2026-08-07 | `dcc283b` leave, `e846355` reimbursement, `05f3a15` employeemovement, `9f7576e` attendance overtime — semua 4 module kini defer ke approval module (flow_id opsional saat submit, approval_instance_id column per module, HandleApprovalStatusChange push handler, NewModuleWithService) |
| Phase 5 - Dokumentasi & Frontend | ✅ Completed | 2026-08-07 | Frontend `Approvals.vue` (tab Flows + My Tasks): flow CRUD dengan module picker dari `available-modules`, step builder (approver type SUPERVISOR/ORGANIZATION/BOTH/ROLE/USER, hierarchy level, organization multiselect, participation type APPROVER/WATCHER, approval mode, reorder via up/down), pending-tasks inbox dengan detail instance + approve/reject/watch-only view. Dokumentasi durable = file plan ini sendiri (sudah ada sejak Phase 1). |
