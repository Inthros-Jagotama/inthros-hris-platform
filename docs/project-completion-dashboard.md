...
| [`module-recruitment-development-plan.md`](module-recruitment-development-plan.md) | 🟡 **Sebagian** | Blueprint Integrated Recruitment **operasional** — **G-1 ✅ + G-2 ✅ + G-3 ✅ (12 Agu 2026)**: G-1 requisition → Central Approval (migration 093, modul `recruitment`, submit + push-callback APPROVED→OPEN); G-2 requisition enhancement (migration 094: `requisition_number` REQ-YYYYMM-XXXXXXXX, `priority` LOW/MEDIUM/HIGH/URGENT default MEDIUM, `position_id`, `opened_at` saat OPEN); G-3 offer management (migration 095 tabel `job_offers`, workflow DRAFT→PENDING_APPROVAL→APPROVED→SENT→ACCEPTED/REJECTED/EXPIRED/WITHDRAWN via Central Approval modul `recruitment_offer`, accept menautkan application ACCEPTED + `slots_filled`++ idempoten, expired guard) — recruitment test 75→**123**. Sisa: G-4..G-12 (Recruitment→Employee/Movement, stage history, FE penuh). Strategic layer **dipisah** ke [`module-recruitment-strategic-layer-plan.md`](module-recruitment-strategic-layer-plan.md) (scoping 12 Agu) |
...
| **Recruitment (ATS)** | **123** | 11.4% | Repository (27), service (68), handler (28) — G-1/G-2/G-3: approval requisition + requisition enhancement + offer management |
...
| | | | |
| **GRAND TOTAL** | **~1595** | **100%** | *(termasuk shared packages & middleware — per-modul lihat kolom Tests di tabel atas)* |
