...
| [`module-recruitment-strategic-layer-plan.md`](module-recruitment-strategic-layer-plan.md) | 🟡 **Sebagian** | Strategic layer ↔ Recruitment (Workforce gap → requisition, expected hires → remaining gap, internal candidate via career path, succession fallback, Quality of Hire, training handoff) — **S-1 workforce gap → requisition ✅ (12 Agu 2026)**: migration 091 `reason_type`/`workforce_gap_id`/`workforce_plan_id`, interface narrow `WorkforceGapProvider` + adapter di main.go, auto-resolve slots, +6 test. Tersisa S-2 s.d. S-7; backend WI & CI sudah ✅ |
