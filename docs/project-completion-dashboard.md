...
| [`module-recruitment-strategic-layer-plan.md`](module-recruitment-strategic-layer-plan.md) | 🟡 **Sebagian** | Strategic layer ↔ Recruitment (Workforce gap → requisition, expected hires → remaining gap, internal candidate via career path, succession fallback, Quality of Hire, training handoff) — **S-1 ✅ + S-2 ✅ + S-4 ✅ (12 Agu 2026)**: S-1 migration 091 + interface narrow `WorkforceGapProvider` + auto-resolve slots; S-2 WI konsumsi pipeline recruitment (`remaining_gap` di gap-analysis & recruitment-analytics), +8 test (WI → 120); S-4 CI eligible internal candidate via career path (`/career-intelligence/paths/{id}/eligible-employees` + interface narrow `InternalCandidateProvider` + `/recruitment/eligible-internal-candidates`), +12 test (CI 75, recruitment 85). Tersisa S-3, S-5, S-6, S-7; backend WI & CI sudah ✅ |
