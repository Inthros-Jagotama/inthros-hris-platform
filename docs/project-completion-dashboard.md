...
| [`module-recruitment-development-plan.md`](module-recruitment-development-plan.md) | 📋 **Proposal** | Blueprint Integrated Recruitment **operasional** (refactor job requisition, integrasi Module Approval/Employee Movement, candidate enhancement, onboarding) — belum dieksekusi; backend ATS dasar sudah ada, FE masih "Coming soon". Strategic layer (Workforce/Career Intelligence) **dipisah** ke [`module-recruitment-strategic-layer-plan.md`](module-recruitment-strategic-layer-plan.md) (scoping 12 Agu) |
| [`module-recruitment-strategic-layer-plan.md`](module-recruitment-strategic-layer-plan.md) | 🟡 **Sebagian** | Strategic layer ↔ Recruitment (Workforce gap → requisition, expected hires → remaining gap, internal candidate via career path, succession fallback, Quality of Hire, training handoff) — **S-1 workforce gap → requisition ✅ + S-2 expected hires → remaining gap ✅ (12 Agu 2026)**: S-1 migration 091 `reason_type`/`workforce_gap_id`/`workforce_plan_id` + interface narrow `WorkforceGapProvider` + auto-resolve slots; S-2 WI konsumsi pipeline recruitment (`expected_hires`/`open_positions`/`filled_positions`/`remaining_gap` di gap-analysis & recruitment-analytics), +8 test (WI 112 → 120). Tersisa S-3 s.d. S-7; backend WI & CI sudah ✅ |
...
| | | | |
| **GRAND TOTAL** | **~1547** | **100%** | *(termasuk shared packages & middleware — per-modul lihat kolom Tests di tabel atas)* |
