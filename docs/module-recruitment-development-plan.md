...
> ⏳ **Sisa TODO (per review 2026-08-13):** G-1 s.d. G-11 selesai; G-12 sebagian (Candidates ✅, summary cards ✅) — sisa gap FE: Applications pipeline board, Interviews, Screening/Assessment.
...
> ✅ **G-12 sub-project 1 selesai (2026-08-13):** halaman Candidates — `Candidates.vue` (list) + `CandidateDetail.vue` (profile 6-tab G-6); tiap sub-tab Create+List+Delete saja (tanpa edit in-place, keputusan brainstorming) — lihat §G-12.
> ✅ **G-12 sub-project 2 selesai (2026-08-13):** summary cards di hub `Recruitment.vue` mengonsumsi `GET /recruitment/analytics/summary` (G-11), fail-silent bila error — lihat §G-12. Applications/Interviews/Screening/Assessment masih menyusul.
...
- **Integrated Recruitment (approval, offer, stage history, screening, assessment, scorecard, candidate enhancement, integrasi operasional)** — 🔶 sebagian: **G-1 approval requisition ✅** + **G-2 requisition enhancement ✅** + **G-3 offer management ✅** + **G-4 Recruitment → Employee/Movement ✅** + **G-5 stage history ✅** + **G-6 candidate enhancement ✅** + **G-7 screening & assessment ✅** + **G-8 interview scorecard ✅** + **G-9 candidate matching ✅** + **G-10 onboarding scoping ✅** + **G-11 analytics ✅** + **G-12 sub-1 Candidates FE ✅** + **G-12 sub-2 summary cards ✅** (2026-08-12/13); sisanya (G-12 Applications/Interviews/Screening/Assessment) rencana (lihat Gap Analysis §7).
...
- Halaman **sudah ada**: `Requisitions.vue` (list/create/edit + submit + priority/requisition_number/opened_at), `Offers.vue` (draft → submit → approval → send → accept/reject/withdraw), `Onboarding.vue` (list employee onboarding + badge "From Offer" + status PENDING→IN_PROGRESS→COMPLETED + create dialog dengan auto-suggest employee dari offer diterima), **`Candidates.vue` + `CandidateDetail.vue`** (G-12 sub-1, 2026-08-13 — list + profile dengan 6 tab G-6, lihat §G-12), `CandidateSearch.vue`, `InternalCandidates.vue`, `RecruitmentAnalytics.vue` (lihat §5.2/strategic layer plan untuk dua yang terakhir).
- `Recruitment.vue` (hub): sudah menampilkan kartu menu ke halaman-halaman di atas (termasuk Candidates); sisanya (Interview, Screening, Assessment, Applications pipeline) masih kartu **"Coming soon"**.
...
- Tidak ada halaman: aplikasi pipeline (kanban board), interview (kalender/scorecard), screening/assessment, analytics summary cards di hub.
...
## G-12 🔶 FRONTEND RECRUITMENT (halaman penuh — sebagian sudah) — sub-project 1 (Candidates) ✅ + sub-project 2 (summary cards) ✅

**Status: 🔶 Sebagian (2026-08-13).** Hub (+ summary cards) + Requisitions + Offers + Onboarding + **Candidates** sudah ada; Applications/Interviews/Screening/Assessment masih "Coming soon" — dipecah menjadi sub-project berurutan (pola G-6/G-7).
...
- **Applications**: pipeline board (Applied → Screening → Assessment → Interview → Final Review → Offer → Hired) — drag/drop hanya memanggil transition service backend; detail + stage history (G-5 ✅) + screening/assessment/interview/offer list + match score (G-9 ✅). Backend G-7/G-8/G-9 sudah lengkap, FE belum.
- **Interviews**: kalender + schedule + interviewer (G-8 ✅) + scorecard (G-8 ✅) + feedback + result.
- **Screening/Assessment**: backend G-7 ✅, FE belum.
