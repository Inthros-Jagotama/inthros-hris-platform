...
> - **REGULAR_GROSS_ANNUALIZED** — metode lama: gross taxable (komponen `is_taxable`), biaya jabatan (min(rate% × gross, max bulanan)), iuran BPJS employee yang boleh dikurangkan (aturan baku: JHT & JP dikurangkan, Kesehatan tidak — migration 122 menghapus toggle `deduct_bpjs_*` dari setting), **pengurang non-BPJS seperti iuran pensiun (komponen bertanda `is_pph21_deductible`, bisa lebih dari satu — migration 119)**, annualisasi, PTKP (dari tabel `ptkps` via status di `employee_tax_profiles` — satu sumber kebenaran, migration 121), PKP (dibulatkan ke bawah ke `pkp_rounding_unit`), bracket progresif (`pph21_tax_brackets`), multiplier non-NPWP (`non_npwp_multiplier_percent`), lalu pajak bulanan.
>
