...
>
> **Map picker untuk lokasi tidak dibangun** — dicek, tidak ada komponen map (Leaflet/Google Maps/dll.) di manapun di `frontend/tenant`, jadi `AttendanceLocations.vue` memakai input numerik manual untuk latitude/longitude, sesuai catatan "cek dulu sebelum menambah dependency baru" di plan FE-2 — menambah library map baru untuk satu form input di luar cakupan phase ini.
>
