...
**Frontend (implementasi 2026-08-15): selesai untuk area inti.** Halaman `Payroll.vue` sekarang menampilkan daftar payroll run + create run + Calculate + aksi status + dashboard; `PayrollRunDetail.vue` dengan tab Employees/Items/Payslips/Payments/Reports. Di Settings tersedia Salary Components, Payroll Periods, Employee Payroll Profiles (payroll/bank/BPJS/tax), BPJS Settings (+ rate components), dan PPh21 Settings (+ PTKP + tax brackets). Masih terbuka di luar scope: payslip PDF, payroll journal, payment reconciliation.
