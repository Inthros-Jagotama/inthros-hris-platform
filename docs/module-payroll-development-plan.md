...
Modul payroll **bukan greenfield di backend** — `backend/internal/modules/payroll/` sudah berisi ±6.289 baris kode, 21 GORM entity, 21 tabel, 48 handler, 71 repository function, 43 test. Awalnya seluruhnya CRUD master data + status-transition sederhana. **Update 2026-08-14/15: calculation engine sudah dibangun** — Formula Engine (`calculator/`), kalkulator BPJS (`bpjs.go`), kalkulator PPh21 (`pph21.go` + `pph21_calculation_logs` terisi), eksekusi "Calculate" pada payroll run (`CalculatePayrollRun`). Yang masih belum ada: payslip generator, payment/bank transfer batch, integrasi workforce — detail lengkap ada di sub-plan di atas.

**Frontend (implementasi 2026-08-15): selesai untuk area inti.** Halaman `Payroll.vue` sekarang menampilkan daftar payroll run + create run + Calculate + aksi status + dashboard; `PayrollRunDetail.vue` dengan tab Employees/Items/Payslips/Payments/Reports. Di Settings tersedia Salary Components, Payroll Periods, BPJS Settings (+ rate components), dan PPh21 Settings (+ PTKP + tax brackets). Employee Payroll Profiles (payroll/bank/BPJS/tax) dipindah ke form karyawan (wizard step "Payroll Profile", 2026-08-16). Masih terbuka di luar scope: payslip PDF, payroll journal, payment reconciliation.
