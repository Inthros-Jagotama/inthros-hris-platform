...
ALTER TABLE pph21_calculation_logs
    ADD COLUMN calculation_method VARCHAR(255) NOT NULL DEFAULT 'REGULAR_GROSS_ANNUALIZED' AFTER tax_method;

-- ---------------------------------------------------------------------------
-- 120.2 ptkps: koreksi kategori TER sesuai aturan resmi
-- ---------------------------------------------------------------------------
UPDATE ptkps SET `group` = 'A' WHERE name LIKE '%(K/0)%';
UPDATE ptkps SET `group` = 'B' WHERE name LIKE '%(TK/2)%';
UPDATE ptkps SET `group` = 'B' WHERE name LIKE '%(TK/3)%';
UPDATE ptkps SET `group` = 'C' WHERE name LIKE '%(K/3)%';
...
SELECT UUID(), 'C', 0, 6600000, 0.00, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM DUAL
