...
ALTER TABLE salary_components
    ADD COLUMN is_pph21_component SMALLINT NOT NULL DEFAULT 0 AFTER print_on_salary_structure,
    ADD COLUMN is_pph21_deductible SMALLINT NOT NULL DEFAULT 0 AFTER is_pph21_component;

-- ---------------------------------------------------------------------------
-- 119.2 pph21_settings: hapus pph21_component_id + FK-nya
-- ---------------------------------------------------------------------------
ALTER TABLE pph21_settings
    DROP FOREIGN KEY fk_pph21_setting_component;

ALTER TABLE pph21_settings
    DROP COLUMN pph21_component_id;

-- ---------------------------------------------------------------------------
-- 119.3 pph21_calculation_logs: pengurang non-BPJS (iuran pensiun dll.)
-- ---------------------------------------------------------------------------
ALTER TABLE pph21_calculation_logs
    ADD COLUMN pension_deductible_monthly DECIMAL(18, 2) NOT NULL DEFAULT 0 AFTER bpjs_tax_deductible_monthly;
