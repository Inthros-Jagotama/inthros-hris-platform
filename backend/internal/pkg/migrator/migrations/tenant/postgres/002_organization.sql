...
CREATE INDEX IF NOT EXISTS idx_orgsum_code ON organization_summaries (code);

CREATE INDEX IF NOT EXISTS idx_orgsum_decree ON organization_summaries (decree_no);

CREATE INDEX IF NOT EXISTS idx_orgsum_deleted_at ON organization_summaries (deleted_at);
...
CREATE INDEX IF NOT EXISTS idx_orglevel_deleted_at ON organization_levels (deleted_at);
...
CREATE INDEX IF NOT EXISTS idx_orgs_zone ON organizations (zone_id);

CREATE INDEX IF NOT EXISTS idx_orgs_job_family ON organizations (job_family_id);
...
CREATE TABLE IF NOT EXISTS job_family_competencies (
    ...
    created_at    TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at    TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
CONSTRAINT uk_jfc_family_competency UNIQUE (job_family_id, competency_id),

    CONSTRAINT fk_jfc_job_family  FOREIGN KEY (job_family_id)  REFERENCES job_families(id)  ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_jfc_competency ON job_family_competencies (competency_id);
...
CREATE TABLE IF NOT EXISTS positions (
    ...
    created_at            TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at            TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
CONSTRAINT uk_positions_org UNIQUE (organization_id),
...
CREATE INDEX IF NOT EXISTS idx_positions_grading ON positions (grading_id);
