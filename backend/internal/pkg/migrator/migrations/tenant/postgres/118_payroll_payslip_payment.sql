...
ALTER TABLE payroll_payslips
    ADD COLUMN total_employer_contribution DECIMAL(18, 2) NOT NULL DEFAULT 0;
...
CREATE TABLE IF NOT EXISTS payroll_payments (
    ...
    -- Snapshot rekening bank (agar perubahan rekening setelah run final
    -- tidak mengubah batch yang sudah dibuat).
    ...
    status                      VARCHAR(255) NOT NULL DEFAULT 'PENDING', -- PENDING, PROCESSING, PAID, FAILED, REVERSED
...
CREATE INDEX IF NOT EXISTS idx_payment_employee ON payroll_payments (employee_id);
CREATE INDEX IF NOT EXISTS idx_payment_status ON payroll_payments (status);
