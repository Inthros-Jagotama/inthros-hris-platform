...
// =============================================================================
// Regression: permission submenu leave.settings.* (admin-config leave: types,
// accrual-policies, reasons) harus benar-benar ter-grant ke company_admin.
//
// Endpoint settings leave digate strict oleh requireLeaveSettings (module
// leave) via permissions CLAIM di JWT. Claim platform user dibangun dari
// rbac_permissions × rbac_role_permissions (useraccount.FindPlatformPermissionsForRole),
// jadi wildcard pada policy hierarchy Enforcer TIDAK cukup — barisnya harus
// ada di database (pola sama dengan rbac_sensitive_fields_test.go).
// =============================================================================

// leaveSettingsPerms adalah permission submenu yang harus dimiliki super_admin
// dan company_admin secara default.
var leaveSettingsPerms = []struct{ resource, action string }{
	{"leave", "settings.view"},
...
func TestSeedDefaults_GrantsLeaveSettingsPermissionsToCompanyAdmin(t *testing.T) {
	...
	for _, role := range []string{string(RoleSuperAdmin), string(RoleCompanyAdmin)} {
		...
		for _, p := range leaveSettingsPerms {
...
// employee di platform hanya view/create biasa (module-level leave.view/create).
func TestSeedDefaults_DoesNotGrantLeaveSettingsPermissionsToManagerOrEmployee(t *testing.T) {
	db, cleanup := setupTestDB()
	defer cleanup()
...
// menerima permission leave.settings.* dan company_admin tetap tidak bisa
// mengelola setting cuti.
func TestUpsertMissingPermissions_BackfillsCompanyAdminLeaveSettings(t *testing.T) {
	db, cleanup := setupTestDB()
	defer cleanup()
	...
	// Simulasikan database lama: hapus permission leave.settings.* beserta grant-nya.
	for _, p := range leaveSettingsPerms {
