...
// Business travel report endpoints reuse requireAttendanceReport("view")
// (permission "attendance.report.view") alih-alih "business_travel.report" —
// permission terakhir dideklarasikan di module.go:100 tapi tidak pernah
// di-seed ke role manapun (lihat migration 149_rbac_submenu_permissions.sql,
// hanya attendance.report.view yang ada), sehingga tidak bisa dipakai tanpa
// migration RBAC baru. attendance.report.view sudah ter-seed & dipakai role
// existing untuk grup Laporan Attendance, jadi laporan business travel
// mengikuti gate yang sama supaya langsung berfungsi.
...
func RegisterRoutes(rg *gin.RouterGroup, handler *Handler) {
	att := rg.Group("/attendance")
	{
		...
		att.GET("/business-travels/reports/travel", requireAttendanceReport("view"), handler.GetBusinessTravelReport)
		att.GET("/business-travels/reports/funding", requireAttendanceReport("view"), handler.GetBusinessTravelFundingReport)
		att.GET("/business-travels/reports/advance", requireAttendanceReport("view"), handler.GetBusinessTravelAdvanceReport)
		att.GET("/business-travels/reports/reimbursement", requireAttendanceReport("view"), handler.GetBusinessTravelReimbursementReport)
		att.GET("/business-travels/reports/refund", requireAttendanceReport("view"), handler.GetBusinessTravelRefundReport)
		att.GET("/business-travels/reports/travel-cost", requireAttendanceReport("view"), handler.GetBusinessTravelCostReport)
