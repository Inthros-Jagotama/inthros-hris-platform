...
// CreateCareerPath membuat career path beserta steps-nya. Validasi: nama unik
// (DB unique index), tiap step punya sequence unik per path, satu posisi tidak
// boleh muncul dua kali dalam path yang sama, dan semua position_id harus
// merujuk posisi yang benar-benar ada (bukan hanya format UUID valid).
func (s *Service) CreateCareerPath(ctx context.Context, req CreateCareerPathRequest) (*CareerPathResponse, error) {
	...
	if req.IsActive != nil {
		path.IsActive = *req.IsActive
	}
	// Generate id di sini (bukan menunggu BeforeCreate) agar steps dapat
	// membawa CareerPathID yang sama sebelum transaksi insert.
	if path.ID == uuid.Nil {
		path.ID = uuid.New()
	}

	steps, err := s.buildAndValidateCareerPathSteps(ctx, path.ID, req.Steps)
	if err != nil {
		return nil, err
	}

	if err := s.repo.CreateCareerPathTx(ctx, path, steps); err != nil {
		return nil, err
	}

	s.logger.Info("Career path created",
		zap.String("career_path_id", path.ID.String()),
		zap.String("name", path.Name),
		zap.Int("steps", len(steps)),
	)

	response := path.ToResponse()
	response.Steps = s.enrichCareerPathSteps(ctx, steps)
	return &response, nil
}

// ListCareerPaths mengembalikan daftar career path (opsional filter keyword
// pada nama) dengan pagination, masing-masing berisi steps terurut sequence.
func (s *Service) ListCareerPaths(ctx context.Context, page, perPage int, keyword string) (*PaginatedCareerPathResponse, error) {
	if page < 1 {
		page = 1
	}
	if perPage < 1 || perPage > 100 {
	...
	paths, total, err := s.repo.ListCareerPaths(ctx, page, perPage, keyword)
	if err != nil {
		return nil, err
	}

	var responses []CareerPathResponse
	for _, p := range paths {
		r := p.ToResponse()
		if steps, err := s.repo.ListCareerPathStepsByPathID(ctx, p.ID); err == nil {
			r.Steps = s.enrichCareerPathSteps(ctx, steps)
		} else {
			s.logger.Warn("failed to load career path steps",
				zap.String("career_path_id", p.ID.String()),
				zap.Error(err),
			)
		}
	...
	return &PaginatedCareerPathResponse{
		Success:    true,
		Data:       responses,
		Page:       page,
		PerPage:    perPage,
...
// GetCareerPathByID mengambil satu career path beserta steps-nya.
func (s *Service) GetCareerPathByID(ctx context.Context, id string) (*CareerPathResponse, error) {
	uid, err := uuid.Parse(id)
	if err != nil {
		return nil, fmt.Errorf("invalid career path id: %w", err)
	}

	path, err := s.repo.FindCareerPathByID(ctx, uid)
	if err != nil {
		return nil, err
	}
...
func (s *Service) UpdateCareerPath(ctx context.Context, id string, req UpdateCareerPathRequest) (*CareerPathResponse, error) {
	...
	// Pastikan path ada sebelum update (404 jelas, bukan FK violation cryptic).
	path, err := s.repo.FindCareerPathByID(ctx, uid)
	if err != nil {
		return nil, err
	}
	...
	path.UpdatedBy = authctx.GetUserID(ctx)

	if err := s.repo.UpdateCareerPathTx(ctx, path, steps); err != nil {
		return nil, err
	}
...
// DeleteCareerPath menghapus career path beserta steps-nya.
func (s *Service) DeleteCareerPath(ctx context.Context, id string) error {
	uid, err := uuid.Parse(id)
	if err != nil {
		return fmt.Errorf("invalid career path id: %w", err)
	}

	return s.repo.DeleteCareerPath(ctx, uid)
}

// buildAndValidateCareerPathSteps membangun model steps dari request dan
// memvalidasinya: minimal 1 step, sequence unik per path, posisi unik per
// path, dan semua position_id merujuk posisi yang ada (plan §12.9).
func (s *Service) buildAndValidateCareerPathSteps(ctx context.Context, careerPathID uuid.UUID, reqSteps []CreateCareerPathStepRequest) ([]CareerPathStep, error) {
	if len(reqSteps) == 0 {
		return nil, &MovementValidationError{Message: "career path must have at least one step"}
	}

	steps := make([]CareerPathStep, 0, len(reqSteps))
	seenSeq := make(map[int]struct{}, len(reqSteps))
	seenPos := make(map[string]struct{}, len(reqSteps))
	positionIDs := make([]uuid.UUID, 0, len(reqSteps))

	for _, rs := range reqSteps {
		if _, dup := seenSeq[rs.Sequence]; dup {
			return nil, &MovementValidationError{Message: fmt.Sprintf("duplicate step sequence: %d", rs.Sequence)}
		}
		seenSeq[rs.Sequence] = struct{}{}

		posID, err := uuid.Parse(rs.PositionID)
		if err != nil {
			return nil, &MovementValidationError{Message: fmt.Sprintf("invalid position id: %s", rs.PositionID)}
		}
		if _, dup := seenPos[posID.String()]; dup {
			return nil, &MovementValidationError{Message: fmt.Sprintf("position %s appears more than once", posID.String())}
		}
		seenPos[posID.String()] = struct{}{}
	...
	if len(names) != len(positionIDs) {
		return nil, &MovementValidationError{Message: "one or more position ids do not exist"}
	}

	return steps, nil
}

// enrichCareerPathSteps mengisi position_name pada response steps dengan satu
// batch query (pola yang sama dengan enrichMovementResponses G-4).
func (s *Service) enrichCareerPathSteps(ctx context.Context, steps []CareerPathStep) []CareerPathStepResponse {
	responses := make([]CareerPathStepResponse, 0, len(steps))
	if len(steps) == 0 {
		return responses
	}
	...
	for _, st := range steps {
		...
		ids = append(ids, st.PositionID)
	}

	if names, err := s.repo.GetPositionNamesByIDs(ctx, ids); err == nil {
		for i := range responses {
			if name, ok := names[responses[i].PositionID]; ok {
				responses[i].PositionName = name
			}
		}
	} else {
		s.logger.Warn("failed to resolve career path step position names", zap.Error(err))
	}
	return responses
}
