...
func (h *Handler) CreateCareerPath(c *gin.Context) {
	...
	response, err := h.service.CreateCareerPath(c.Request.Context(), req)
	if err != nil {
		if status, ok := movementErrStatus(err); ok {
			httputil.ErrorRaw(c, status, movementErrCode(status), err.Error())
			return
...
// ListCareerPaths menangani GET /api/v1/tenant/career-paths?page=&per_page=&keyword=
func (h *Handler) ListCareerPaths(c *gin.Context) {
	page, _ := strconv.Atoi(c.DefaultQuery("page", "1"))
	perPage, _ := strconv.Atoi(c.DefaultQuery("per_page", "20"))
	keyword := c.Query("keyword")

	response, err := h.service.ListCareerPaths(c.Request.Context(), page, perPage, keyword)
	if err != nil {
		httputil.InternalError(c, err.Error())
		return
	}
...
// GetCareerPathByID menangani GET /api/v1/tenant/career-paths/:id
func (h *Handler) GetCareerPathByID(c *gin.Context) {
	id := c.Param("id")

	response, err := h.service.GetCareerPathByID(c.Request.Context(), id)
	if err != nil {
		httputil.NotFound(c, err.Error())
		return
	}
...
// UpdateCareerPath menangani PUT /api/v1/tenant/career-paths/:id
func (h *Handler) UpdateCareerPath(c *gin.Context) {
	...
	response, err := h.service.UpdateCareerPath(c.Request.Context(), id, req)
	if err != nil {
		if status, ok := movementErrStatus(err); ok {
			httputil.ErrorRaw(c, status, movementErrCode(status), err.Error())
			return
...
// DeleteCareerPath menangani DELETE /api/v1/tenant/career-paths/:id
func (h *Handler) DeleteCareerPath(c *gin.Context) {
	id := c.Param("id")

	if err := h.service.DeleteCareerPath(c.Request.Context(), id); err != nil {
		httputil.InternalError(c, err.Error())
		return
	}
	httputil.DeletedJSON(c, "success.deleted")
