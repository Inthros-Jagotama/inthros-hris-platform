...
func seedReligions(db *gorm.DB) (int, int, error) {
	data := []map[string]interface{}{
		{"id": codeToUUID("religion", "ISL"), "code": "ISL", "religion": "Islam", "sort_order": 1, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("religion", "KTL"), "code": "KTL", "religion": "Kristen", "sort_order": 2, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("religion", "KTK"), "code": "KTK", "religion": "Katolik", "sort_order": 3, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("religion", "HND"), "code": "HND", "religion": "Hindu", "sort_order": 4, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("religion", "BDH"), "code": "BDH", "religion": "Budha", "sort_order": 5, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("religion", "KHC"), "code": "KHC", "religion": "Konghucu", "sort_order": 6, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("religion", "LNY"), "code": "LNY", "religion": "Lainnya", "sort_order": 7, "created_at": time.Now(), "updated_at": time.Now()},
	}
...
func seedEducations(db *gorm.DB) (int, int, error) {
	data := []map[string]interface{}{
		{"id": codeToUUID("education", "TK"), "code": "TK", "education": "Taman Kanak-Kanak", "sort_order": 1, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "SD"), "code": "SD", "education": "Sekolah Dasar", "sort_order": 2, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "SMP"), "code": "SMP", "education": "Sekolah Menengah Pertama", "sort_order": 3, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "SMA"), "code": "SMA", "education": "Sekolah Menengah Atas", "sort_order": 4, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "SMK"), "code": "SMK", "education": "Sekolah Menengah Kejuruan", "sort_order": 5, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "D1"), "code": "D1", "education": "Diploma 1", "sort_order": 6, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "D2"), "code": "D2", "education": "Diploma 2", "sort_order": 7, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "D3"), "code": "D3", "education": "Diploma 3", "sort_order": 8, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "D4"), "code": "D4", "education": "Diploma 4", "sort_order": 9, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "S1"), "code": "S1", "education": "Strata 1", "sort_order": 10, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "S2"), "code": "S2", "education": "Strata 2", "sort_order": 11, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("education", "S3"), "code": "S3", "education": "Strata 3", "sort_order": 12, "created_at": time.Now(), "updated_at": time.Now()},
	}
...
func seedMaritalStatuses(db *gorm.DB) (int, int, error) {
	data := []map[string]interface{}{
		{"id": codeToUUID("marital_status", "BLM"), "code": "BLM", "marital_status": "Belum Kawin", "sort_order": 1, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("marital_status", "KWN"), "code": "KWN", "marital_status": "Kawin", "sort_order": 2, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("marital_status", "CRH"), "code": "CRH", "marital_status": "Cerai Hidup", "sort_order": 3, "created_at": time.Now(), "updated_at": time.Now()},
		{"id": codeToUUID("marital_status", "CRM"), "code": "CRM", "marital_status": "Cerai Mati", "sort_order": 4, "created_at": time.Now(), "updated_at": time.Now()},
	}
...
// seedFromSQLFile membaca file .sql dan mengeksekusinya ke database.
// Disable FK checks sementara karena data master lintas-referensi.
func seedFromSQLFile(db *gorm.DB, filePath string) (int, int, error) {
	data, err := os.ReadFile(filePath)
	if err != nil {
		return 0, 0, fmt.Errorf("read SQL file %s failed: %w", filePath, err)
	}
	...
	if err := db.Exec(sql).Error; err != nil {
		...
		return 0, 0, fmt.Errorf("execute SQL %s failed: %w", filePath, err)
	}
...
// seedLargeSQL men-seed data dari file SQL yang besar (districts/villages).
// Mencari file di working directory atau relative paths dari project root.
func seedLargeSQL(db *gorm.DB, tableName, fileName string) (int, int, error) {
	// Cari file SQL di relative paths (dari working directory)
	candidates := []string{
		filepath.Join("backend", "migrations", "seeders", fileName),
		filepath.Join("migrations", "seeders", fileName),
		filepath.Join("..", "migrations", "seeders", fileName),
		filepath.Join("..", "backend", "migrations", "seeders", fileName),
	}
	...
	for _, p := range candidates {
		...
		if _, err := os.Stat(fullPath); err == nil {
			return seedFromSQLFile(db, fullPath)
		}
	}

	return 0, 0, fmt.Errorf("%s SQL file not found (searched: %v)", tableName, candidates)
}

// seedDistrictsFromSQL men-seed districts dari file SQL.
func seedDistrictsFromSQL(db *gorm.DB) (int, int, error) {
	return seedLargeSQL(db, "districts", "002_seed_districts.sql")
}

// seedVillagesFromSQL men-seed villages dari file SQL.
func seedVillagesFromSQL(db *gorm.DB) (int, int, error) {
	return seedLargeSQL(db, "villages", "003_seed_villages.sql")
}
