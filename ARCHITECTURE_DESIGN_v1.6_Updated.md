...
| **Settings (Master Data Reference)** | **17 GORM entities** | ❌ | ✅ **Completed (27 Jul 2026)** — Full CRUD untuk 17 reference entities di `backend/internal/modules/setting/`: Zones (Wilayah), Provinces (Provinsi), Regencies (Kabupaten/Kota), Districts (Kecamatan), Villages (Desa/Kelurahan), Educations (Pendidikan), Religions (Agama), MaritalStatuses (Status Pernikahan), RelationshipTypes (Tipe Hubungan), Banks (Bank), EmploymentStatuses (Status Kepegawaian), Nationalities (Kebangsaan), JobFamilies (Grup Jabatan), SalaryGrades (Grade Gaji — dengan Grade, MinSalary, MaxSalary), TER (Tarif Efektif Rata-rata), PTKP (Penghasilan Tidak Kena Pajak), **Insurances (Asuransi — Code, Name, SortOrder)**. **85 OpenAPI endpoints** (5 per entity). Query params: `?page=&per_page=&search=&sort_by=&sort_order=`. Zones tambahan: `?active_only=true`. **Frontend:** 17 views di `frontend/tenant/src/views/settings/` — semuanya menggunakan pattern Companies.vue. **Table:** `insurances` dibuat manual via SQL karena GORM AutoMigrate tidak berjalan (server tidak di-restart setelah model ditambahkan). |
...
* [x] **Career Intelligence & Talent Management:** ✅ **Completed (26 Jul 2026)** — Strategic talent analytics module for 9-box talent mapping (CareerTalentMap), career interests tracking (CareerInterest), career path gap analysis (CareerPath), and succession planning (CareerSuccessionPlan). **4 GORM entities, 65 unit tests** (23 repository + 20 service + 22 handler). **19 OpenAPI endpoints** across 4 resource groups: Talent Maps (7), Career Interests (3), Career Paths (4), Succession Plans (5). Full OpenAPI documentation with 27 tags, migration files 018 (4 new tables).
* [x] **Settings (Master Data Reference):** ✅ **Completed (27 Jul 2026)** — Backend CRUD untuk 16 reference entities (Zones, Provinces, Regencies, Districts, Villages, Educations, Religions, MaritalStatuses, RelationshipTypes, Banks, EmploymentStatuses, Nationalities, JobFamilies, SalaryGrades, TER, PTKP). Semua di `backend/internal/modules/setting/` dengan struktur standar (model, dto, repository, service, handler, routes, module). **80 OpenAPI endpoints** (5 per entity). Query params: `?page=&per_page=&search=&sort_by=&sort_order=` untuk server-side pagination & sorting. **Frontend:** 16 setting views di `frontend/tenant/src/views/settings/` — semua menggunakan pattern Companies.vue yang sudah terbukti (search bar, DataTable styling, skeletonColumns, filteredItems computed, dialog section header icons, v-tooltip.left actions, sortable columns, bilingual EN/ID, dark mode).
...
| **Settings** | Settings — 17 Reference CRUDs (+Insurances) | ✅ **Done (27 Jul 2026)** | 17 GORM entities di `backend/internal/modules/setting/`: Zones, Provinces, Regencies, Districts, Villages, Educations, Religions, MaritalStatuses, RelationshipTypes, Banks, EmploymentStatuses, Nationalities, JobFamilies, SalaryGrades, TER, PTKP, **Insurances**. **85 OpenAPI endpoints** (5 per entity). **Frontend:** 17 views dengan pattern Companies.vue — search bar, DataTable styling, skeleton, bilingual, dark mode. **Table:** `insurances` dibuat manual via SQL. |
