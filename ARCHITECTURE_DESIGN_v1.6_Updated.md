...
| **Payroll & Compensation Engine** | **21 GORM entities** | ✅ **34 tests** | ✅ **Completed (24 Juli 2026)** — BPJS, PPh21, Payroll Run, Employee Profiles. **53 OpenAPI endpoints** |
...
| **Settings (Master Data Reference)** | **19 GORM entities** | ❌ | ✅ **Completed (27 Jul 2026, +Company Holidays 1 Aug 2026)** — Full CRUD untuk 19 reference entities di `backend/internal/modules/setting/`: Zones (Wilayah), Provinces (Provinsi), Regencies (Kabupaten/Kota), Districts (Kecamatan), Villages (Desa/Kelurahan), Educations (Pendidikan), Education Majors (Jurusan Pendidikan), Religions (Agama), MaritalStatuses (Status Pernikahan), RelationshipTypes (Tipe Hubungan), Banks (Bank), EmploymentStatuses (Status Kepegawaian), Nationalities (Kebangsaan), JobFamilies (Grup Jabatan), SalaryGrades (Grade Gaji — dengan Grade, MinSalary, MaxSalary), TER (Tarif Efektif Rata-rata), PTKP (Penghasilan Tidak Kena Pajak), **Insurances (Asuransi — Code, Name, SortOrder)**, **Company Holidays (Hari Libur Perusahaan — HolidayDate, Name, Description, IsActive)**. **95 OpenAPI endpoints** (5 per entity). Query params: `?page=&per_page=&search=&sort_by=&sort_order=`. Zones tambahan: `?active_only=true`. **Frontend:** 19 views di `frontend/tenant/src/views/settings/` — semuanya menggunakan pattern Companies.vue. **Table:** `insurances` dibuat manual via SQL karena GORM AutoMigrate tidak berjalan (server tidak di-restart setelah model ditambahkan). **Seeder:** `seedEducationMajors` pakai kode 3 digit (001–020), 20 jurusan, UUID deterministik + idempotent. |
...
* [x] **Career Intelligence & Talent Management:** ✅ **Completed (26 Jul 2026)** — Strategic talent analytics module for 9-box talent mapping (CareerTalentMap), career interests tracking (CareerInterest), career path gap analysis (CareerPath), and succession planning (CareerSuccessionPlan). **4 GORM entities, 65 unit tests** (23 repository + 20 service + 22 handler). **19 OpenAPI endpoints** across 4 resource groups: Talent Maps (7), Career Interests (3), Career Paths (4), Succession Plans (5). Full OpenAPI documentation with 30 tags, migration files 018 (4 new tables).
* [x] **Settings (Master Data Reference):** ✅ **Completed (27 Jul 2026)** — Backend CRUD untuk 18 reference entities (Zones, Provinces, Regencies, Districts, Villages, Educations, Education Majors, Religions, MaritalStatuses, RelationshipTypes, Banks, EmploymentStatuses, Nationalities, JobFamilies, SalaryGrades, TER, PTKP, Insurances). Semua di `backend/internal/modules/setting/` dengan struktur standar (model, dto, repository, service, handler, routes, module). **90 OpenAPI endpoints** (5 per entity). Query params: `?page=&per_page=&search=&sort_by=&sort_order=` untuk server-side pagination & sorting. **Frontend:** 18 setting views di `frontend/tenant/src/views/settings/` — semua menggunakan pattern Companies.vue yang sudah terbukti (search bar, DataTable styling, skeletonColumns, filteredItems computed, dialog section header icons, v-tooltip.left actions, sortable columns, bilingual EN/ID, dark mode).
...
| **Payroll**| Payroll & Compensation Engine | ✅ **Done** | 21 GORM entities, full CRUD, 34 unit tests, **53 OpenAPI endpoints**. BPJS, PPh21, Payroll Run, Employee Payroll Profiles. |
...
| **Settings** | Settings — 19 Reference CRUDs (+Insurances, +Education Majors, +Company Holidays) | ✅ **Done (27 Jul 2026, +Company Holidays 1 Aug 2026)** | 19 GORM entities di `backend/internal/modules/setting/`: Zones, Provinces, Regencies, Districts, Villages, Educations, Education Majors, Religions, MaritalStatuses, RelationshipTypes, Banks, EmploymentStatuses, Nationalities, JobFamilies, SalaryGrades, TER, PTKP, **Insurances**, **Company Holidays**. **95 OpenAPI endpoints** (5 per entity). **Frontend:** 19 views dengan pattern Companies.vue — search bar, DataTable styling, skeleton, bilingual, dark mode. **Catatan:** `company_holidays` tabel tanpa updated_at/deleted_at → Update `Updates(map)` + Delete hard-delete; model IsActive tanpa tag `default:true` (gotcha GORM); fix bilingual: label field description pakai key `desc` terpisah dari page description. |

---
*Document Version: 1.6-Updated (v16)*  
*Status: Approved for Architecture Enhancement*

### Changelog

| Versi | Tanggal | Perubahan |
| v26 | 02 Aug 2026 | ✅ **Icon fix Job Value Mapping (PrimeIcons 8.0.0 — FE tenant).** `JobValuesIndex.vue` memakai 4 ikon yang **tidak ada** di PrimeIcons 8.0.0 → ikon tidak render (kosong): `pi-brain` (Lingkungan Berpikir + Kecerdasan) → `pi-wave-pulse` / `pi-microchip`; `pi-puzzle` (Tantangan Berpikir) → `pi-flag`; `pi-target` (Achievement Orientation) → `pi-bullseye`; `pi-handshake` (Building Partnership) → `pi-link`. Audit semua 37 ikon unik vs `primeicons.css` → 0 missing tersisa. Build FE ✅ + browser test ✅ (7 grup card render, 0 console errors). Changelog dashboard & frontend-development-plan disinkronkan |
| v25 | 02 Aug 2026 | ✅ **Job Value UUID standardization (migration 043).** `job_management_values` — semua **306 baris distandarkan ke UUID v4 acak proper** (kolom char(36)). Latar: seed 033-042 memakai UUID tetap/serial (`aaaaaaaa-0001-4000-8000-...`) demi idempotensi `WHERE NOT EXISTS`; user minta standar UUID proper. Migration **043** (mysql + postgres + down) mengubah semua id pola tetap `-4000-8000-` → UUID v4 (MySQL: ekspresi `RANDOM_BYTES`-based v4; Postgres: `gen_random_uuid()`), berlaku untuk DB existing MAUPUN fresh install. Down migration mengembalikan id tetap seed dengan mencocokkan (type, level, descriptions, sort) — kombinasi unik per row. Tanpa FK & child tables kosong → aman tanpa dangling reference. **Bug fix penting:** versi awal memakai `MOD(ABS(RAND()*100), 4)` yang saat RAND ≈ 0.9999 menghasilkan posisi `SUBSTR` 5 → segmen ke-4 hanya 3 karakter (UUID 35-char, 31 row malformed) — diperbaiki ke `FLOOR(RAND()*4)` (integer 0-3, 30x test valid). Verifikasi: 306/306 proper v4, 0 duplikat, API normal, go build ✅ |
| v20 | 01 Aug 2026 | ✅ **TenantResolver Middleware — auto-detect company dari Host header (mode SaaS) + FE sync X-Tenant-ID.** Backend: middleware baru `middleware.TenantResolver` (interface `HostCompanyResolver`): prioritas JWT `company_id` (menang — cegah cross-tenant) → header `X-Tenant-ID` (divalidasi UUID + normalisasi kanonik) → `X-Forwarded-Host` → `Host` → resolve via company module (`companyModule.ResolveByHost`); set gin context `company_id` + **response header `X-Tenant-ID`**. Dipasang di tenant group (`router.Setup` param baru, nil-safe) antara AuthJWT & TenantRequired, juga di group login/refresh — FE **tidak perlu mengirim company_slug/company_id manual**. Login handler fallback ke `company_id` dari context saat body kosong. Frontend tenant: state `company` di auth store (persist localStorage `tenant_company`), `setCompany()`; api.js request kirim header `X-Tenant-ID` + response interceptor **baca `X-Tenant-ID` → sync state company otomatis**; Login.vue company opsional (+locale `auth.login.company_optional`) & pass companyName; Sidebar companyLabel prioritas state company. 7 unit test middleware (no-host, resolve host, header precedence, unresolvable, X-Forwarded-Host precedence, invalid header ignored, JWT wins). Total: **677 endpoints, 368 paths, 433 schemas, 27 tags**. |
| v21 | 01 Aug 2026 | ✅ **Education Majors setting + seeder kode 3 digit.** Backend: model `EducationMajor` + full CRUD (repo/service/handler/routes/module) di `backend/internal/modules/setting/` — 4 permission `setting.education_major.*`, menu 'Education Majors', AutoMigrate. Migration **024_education_majors.sql** (mysql+pg+down). Seeder `seedEducationMajors` — **kode 3 digit (001–020)**, 20 jurusan, UUID deterministik `codeToUUID`, idempotent `batchInsert`. Frontend: `EducationMajorsView.vue` + route + SettingsIndex card + locale keys EN/ID. OpenAPI: 5 endpoints injected (GET/POST list, GET/PUT/DELETE by id), 3 schemas. Total: **684 endpoints, 371 paths, 439 schemas, 30 tags**. 18 Settings entities total. |
| v22 | 01 Aug 2026 | ✅ **Company Holidays CRUD (Settings) + fix bilingual desc.** Backend: model `CompanyHoliday` + full CRUD di `backend/internal/modules/setting/` — 4 permission `setting.company_holiday.*`, menu 'Company Holidays', AutoMigrate. **Catatan teknis:** tabel `company_holidays` TANPA kolom updated_at/deleted_at → Update pakai `Updates(map)`, Delete hard-delete (`Unscoped`); model `IsActive` sengaja tanpa tag `default:true` (gotcha GORM false→true). Frontend: `CompanyHolidaysView.vue` (DateInput + ToggleSwitch + ConfirmDeleteDialog) + route + SettingsIndex card + locale EN/ID; **fix bilingual** — label field description pakai key `desc`/`desc_placeholder` terpisah dari page description. OpenAPI: 5 endpoints + 3 schemas injected (+fix hapus path-level `tags` tidak valid di `/{id}`). Total: **689 endpoints, 373 paths, 442 schemas, 30 tags** (report v16). 19 Settings entities total. |
| v23 | 01 Aug 2026 | ✅ **Company Holidays — Calendar View + Badge Fix (FE tenant).** `CompanyHolidaysView.vue` — Toggle Table/Calendar (pola Organizations.vue): DatePicker inline (primevue ^4.5.5) + slot `#date` badge rose penuh (`bg-rose-500 text-white`) di angka tanggal libur + panel legend "Holidays This Month" + klik tanggal → ada libur: edit, belum ada: modal tambah dengan tanggal terisi (`openDialog(null, key)` — `editing=false`, mode tambah murni). `viewYear/viewMonth` (0-indexed) ikut `@month-change`/`@year-change` (payload `{ month: 1-indexed, year }`) agar legend tidak basi saat navigasi. `loadCalendar()` fetch `per_page=500` (= maxPerPage backend) + di-refresh setelah save/delete. **Bug fix ×2:** (1) slot `#date` PrimeVue v4 menerima OBJECT `{ day, month 0-indexed, year }` (bukan instance Date) → template pakai `date.day` + `toDateKey()` dual-mode (Date & object slot) — mencegah `TypeError: date.getDate is not a function`; (2) normalisasi `normDateKey()` (`slice(0,10)` → selalu `YYYY-MM-DD`) di `holidayMap` + filter/sort `monthHolidays` agar badge tetap tampil apa pun format `holiday_date` dari API (dengan waktu/TZ). Total endpoint tidak berubah (FE-only): **689 endpoints, 373 paths, 442 schemas, 30 tags**. |
| v25 | 05 Aug 2026 | ✅ **Job Management — auto score calculator + docs sync.** Backend: `calculator.go` (port `JobValueCalculator.php`: 10 komponen, mapping 5 tabel, `MAP_COMMUNICATION {1:1,2:3,3:6}`, batch `CalculateForOrganizationIDs`); **hook recalc otomatis** di Create/Update/Delete tiap section yang memengaruhi skor; `is_complete`/`completed_at` dipersist (migration 051); kolom `type_group` & `description_group` (052/053) + tabel `job_management_value_clusters` (054); endpoint baru `GET /job-management/values/tree`, `GET/PUT /job-management/values/clusters/:type`, 5 endpoint relationship details; param `?search=` di API organizations (filter code/full_code/nomenclature, prefix tabel `organizations.` cegah error 1052 ambiguous). FE: form multi-section `JobManagementForm.vue` + sticky `JobScoreSummary`; card potensi (Psychological/Skill/ProblemSolving/Technical bobot %/Managerial) + composable `usePotencyLevels`; `JobScoreSection.vue` breakdown dari field `components`; daftar job management kolom Score/With Financial (Yes-No)/Status + pencarian (tanpa kolom code/order/level). Total: **698 endpoints, 378 paths, 442 schemas, 30 tags** (report v17). Migration tenant: 106 file per dialect (53 up + 53 down). |
| v24 | 02 Aug 2026 | ✅ **Org Tree drag & drop (pindah parent) — FE tenant (PrimeVue 4.5.5 API fix).** `Organizations.vue` mode Tree: PrimeVue Tree `:draggable-nodes`/`:droppable-nodes`/`validate-drop` + `@node-drop` — payload asli v4.5.5: `dragNode`/`dropNode`/`dropPosition`/`accept`. Parent baru dari `dropPosition` (0 = drop ON node → parent = dropNode; ±1 = drop sebelum/sesudah → sibling → parent = parent dropNode; dropNode null = drop ke root → parent = null). Guard anti-siklus (self/descendant) → toast error + `accept()` (reset state drag, tanpa ghost) + reload; parent sama → `accept()` + reload; parent beda → `accept()` + `PUT /organizations/:id { parent_id }` + toast sukses + reload; `moving` ref sebagai guard concurrency (drag dinonaktifkan selama proses). **Bug fix (akar masalah):** kode lama pakai props `:draggable`/`droppable` + `event.node`/`event.dropIndex` — API tersebut TIDAK ada di PrimeVue 4.5.5 (props asli: `draggableNodes`/`droppableNodes`/`validateDrop`), sehingga atribut jatuh sebagai HTML attribute dan drag tidak pernah teraktivasi. Pendukung: dialog edit kini menyertakan dropdown parent (eksklusi self + descendant), `OrgTreeTable.vue` recursive (semua level), backend Update mendukung pindah parent (full_code/level/propagasi descendant). Total endpoint tidak berubah (FE-only): **690 endpoints, 374 paths, 442 schemas, 30 tags**. Build FE ✅ |
| v19 | 01 Aug 2026 | ✅ **Domain/Subdomain per Company (mode SaaS) + resolve-by-hostname + tenant login `company_id`.** Migration **013_add_companies_domain** (+down): kolom `subdomain`/`domain` + UNIQUE INDEX. Company CRUD + field baru; `Service.ResolveByHost` (normalisasi host → domain → subdomain, hanya status active, helper `subdomainFromHost`/`isAllDigits`/`emptyToNil` — string kosong → NULL cegah bentrok unique ''), pre-check duplikat subdomain/domain. Endpoint publik **GET /api/v1/public/companies/resolve?host=...** (module `PublicHandler`). Tenant login terima `company_id` (dev env `VITE_COMPANY_ID`) selain `company_slug`. FE tenant `Login.vue` auto-detect subdomain dari URL → resolve → prefill. Platform-admin: field subdomain/domain di create/edit form (Companies.vue + CompanyActions.vue). 6 unit test baru (subdomainFromHost, ResolveByHost 5 skenario, emptyToNil, NULL-storage Create/Update, duplikat ditolak). Total: **677 endpoints, 368 paths, 433 schemas, 27 tags**. |
| v18 | 31 Jul 2026 | ✅ **Rotasi Kredensial DB Tenant (endpoint backend).** `database.Manager.RotateTenantCredentials` (ALTER USER dialect-aware MySQL/PG + update `tenant_connections` terenkripsi AES-256-GCM + close cached conn), `Service.RotateCredentials` (auto-generate password 24 char, guard anti SQL injection, warning root/superuser), endpoint **POST /api/v1/platform/companies/{id}/rotate-credentials** (409 `ROTATE_FAILED`), 8 unit test. Docs: deployment-guide 3.9 via API + openapi.json (+1 path, +2 schemas). Total: **674 endpoints, 365 paths, 431 schemas, 27 tags**. |
| v14 | 28 Jul 2026 | ✅ **OpenAPI sync — 9 missing endpoints injected.** Settings: +10 endpoints (TER, PTKP). Monitoring: +2 endpoints (pool, seed-status). Payroll: +6 endpoints (pph21-ptkp-rates/{id}, pph21-tax-brackets/{id}). Tenant: +1 endpoint (company-modules). Total: **635 endpoints, 422 schemas, 27 tags**. |
| v16 | 30 Jul 2026 | ✅ **15 missing endpoints injected.** Employee: +4 (photo upload/delete, documents upload, documents replace). Settings: +10 (gradings CRUD, villages search/detail, provinces/regencies/districts cascading). Platform: +2 (user password change, user delete). Total: **665 endpoints, 360 paths, 426 schemas, 27 tags**. |
| v17 | 30 Jul 2026 | ✅ **Insurances CRUD added + OpenAPI injection.** Backend: model `Insurance` + full CRUD (repo/service/handler/routes/module) di `backend/internal/modules/setting/`. Frontend: `InsurancesView.vue` + route + sidebar + locale keys EN/ID. OpenAPI: 5 endpoints injected (GET/POST list, GET/PUT/DELETE by id), 3 schemas (Create/Update/Response). **Table `insurances`** dibuat manual via SQL (`CREATE TABLE ...`) karena server tidak di-restart setelah model ditambahkan — tabel memiliki kolom: id (CHAR(36) PK), code, name, sort_order, created_by, updated_by, timestamps, deleted_at. Total: **673 endpoints, 364 paths, 429 schemas, 27 tags**. 17 Settings entities total. |
| v15 | 28 Jul 2026 | ✅ **Organization Summary CRUD — 6 endpoints injected.** Organizations: 12→18 endpoints (+6 for organization-summaries CRUD). New schemas: OrganizationSummaryResponse, PaginatedSummaryResponse, CreateOrganizationSummaryRequest, UpdateOrganizationSummaryRequest. Frontend Organization Summary page now reads/writes from `organization_summaries` table. Total: **651 endpoints, 349 paths, 426 schemas, 27 tags**. |
| v13 | 27 Jul 2026 | ✅ **Settings (Master Data Reference)** selesai — 14 GORM entities di `backend/internal/modules/setting/`. **70 OpenAPI endpoints**. ✅ **DependsOn updated** — 6 tenant modules menambahkan `"setting"` di DependsOn (Employee, Payroll, Recruitment, Performance, Training, Employee Movement) karena referensi data Settings (Education, Religion, MaritalStatus, Bank, EmploymentStatus). ✅ **Seeder & DB** — Module `setting` didaftarkan sebagai tenant module ke-16 di `cmd/installer/main.go` dan database. **Frontend:** 14 settings views pattern Companies.vue. |
