...
| **Settings (Master Data Reference)** | **16 GORM entities** | ❌ | ✅ **Completed (27 Jul 2026)** — Full CRUD untuk 16 reference entities di `backend/internal/modules/setting/`: Zones (Wilayah), Provinces (Provinsi), Regencies (Kabupaten/Kota), Districts (Kecamatan), Villages (Desa/Kelurahan), Educations (Pendidikan), Religions (Agama), MaritalStatuses (Status Pernikahan), RelationshipTypes (Tipe Hubungan), Banks (Bank), EmploymentStatuses (Status Kepegawaian), Nationalities (Kebangsaan), JobFamilies (Grup Jabatan), SalaryGrades (Grade Gaji — dengan Grade, MinSalary, MaxSalary), TER (Tarif Efektif Rata-rata), PTKP (Penghasilan Tidak Kena Pajak). **80 OpenAPI endpoints** (5 per entity). Query params: `?page=&per_page=&search=&sort_by=&sort_order=`. Zones tambahan: `?active_only=true`. **Frontend:** 16 views di `frontend/tenant/src/views/settings/` — semuanya menggunakan pattern Companies.vue. |
...
| **Settings** | Settings — 16 Reference CRUDs | ✅ **Done (27 Jul 2026)** | 16 GORM entities di `backend/internal/modules/setting/`: Zones, Provinces, Regencies, Districts, Villages, Educations, Religions, MaritalStatuses, RelationshipTypes, Banks, EmploymentStatuses, Nationalities, JobFamilies, SalaryGrades, TER, PTKP. **80 OpenAPI endpoints** (5 per entity). **Frontend:** 16 views dengan pattern Companies.vue — search bar, DataTable styling, skeleton, bilingual, dark mode. |

---
*Document Version: 1.6-Updated (v15)*  
