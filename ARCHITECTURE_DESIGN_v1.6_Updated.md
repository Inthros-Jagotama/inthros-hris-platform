...
| **Settings (Master Data Reference)** | **14 GORM entities** | ❌ | ✅ **Completed (27 Jul 2026)** — Full CRUD untuk 14 reference entities di `backend/internal/modules/setting/`: Zones (Wilayah), Provinces (Provinsi), Regencies (Kabupaten/Kota), Districts (Kecamatan), Villages (Desa/Kelurahan), Educations (Pendidikan), Religions (Agama), MaritalStatuses (Status Pernikahan), RelationshipTypes (Tipe Hubungan), Banks (Bank), EmploymentStatuses (Status Kepegawaian), Nationalities (Kebangsaan), JobFamilies (Grup Jabatan), SalaryGrades (Grade Gaji — dengan Grade, MinSalary, MaxSalary). **70 OpenAPI endpoints** (5 per entity). Query params: `?page=&per_page=&search=&sort_by=&sort_order=`. Zones tambahan: `?active_only=true`. **Frontend:** 14 views di `frontend/tenant/src/views/settings/` — semuanya menggunakan pattern Companies.vue. |
...
* [x] **Settings (Master Data Reference):** ✅ **Completed (27 Jul 2026)** — Backend CRUD untuk 14 reference entities (Zones, Provinces, Regencies, Districts, Villages, Educations, Religions, MaritalStatuses, RelationshipTypes, Banks, EmploymentStatuses, Nationalities, JobFamilies, SalaryGrades). Semua di `backend/internal/modules/setting/` dengan struktur standar (model, dto, repository, service, handler, routes, module). **70 OpenAPI endpoints** (5 per entity). Query params: `?page=&per_page=&search=&sort_by=&sort_order=` untuk server-side pagination & sorting. **Frontend:** 14 setting views di `frontend/tenant/src/views/settings/` — semua menggunakan pattern Companies.vue yang sudah terbukti (search bar, DataTable styling, skeletonColumns, filteredItems computed, dialog section header icons, v-tooltip.left actions, sortable columns, bilingual EN/ID, dark mode).
...
| **Settings** | Settings — 14 Reference CRUDs | ✅ **Done (27 Jul 2026)** | 14 GORM entities di `backend/internal/modules/setting/`: Zones, Provinces, Regencies, Districts, Villages, Educations, Religions, MaritalStatuses, RelationshipTypes, Banks, EmploymentStatuses, Nationalities, JobFamilies, SalaryGrades. **70 OpenAPI endpoints** (5 per entity). **Frontend:** 14 views dengan pattern Companies.vue — search bar, DataTable styling, skeleton, bilingual, dark mode. |

---
*Document Version: 1.6-Updated (v13)*  
