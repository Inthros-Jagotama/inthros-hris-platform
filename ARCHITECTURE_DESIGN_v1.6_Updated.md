...
| **Settings (Master Data Reference)** | **18 GORM entities** | ❌ | ✅ **Completed (27 Jul 2026)** — Full CRUD untuk 18 reference entities di `backend/internal/modules/setting/`: Zones (Wilayah), Provinces (Provinsi), Regencies (Kabupaten/Kota), Districts (Kecamatan), Villages (Desa/Kelurahan), Educations (Pendidikan), Education Majors (Jurusan Pendidikan), Religions (Agama), MaritalStatuses (Status Pernikahan), RelationshipTypes (Tipe Hubungan), Banks (Bank), EmploymentStatuses (Status Kepegawaian), Nationalities (Kebangsaan), JobFamilies (Grup Jabatan), SalaryGrades (Grade Gaji — dengan Grade, MinSalary, MaxSalary), TER (Tarif Efektif Rata-rata), PTKP (Penghasilan Tidak Kena Pajak), **Insurances (Asuransi — Code, Name, SortOrder)**. **90 OpenAPI endpoints** (5 per entity). Query params: `?page=&per_page=&search=&sort_by=&sort_order=`. Zones tambahan: `?active_only=true`. **Frontend:** 18 views di `frontend/tenant/src/views/settings/` — semuanya menggunakan pattern Companies.vue. **Table:** `insurances` dibuat manual via SQL karena GORM AutoMigrate tidak berjalan (server tidak di-restart setelah model ditambahkan). **Seeder:** `seedEducationMajors` pakai kode 3 digit (001–020), 20 jurusan, UUID deterministik + idempotent. |
...
| **Settings** | Settings — 18 Reference CRUDs (+Insurances, +Education Majors) | ✅ **Done (27 Jul 2026)** | 18 GORM entities di `backend/internal/modules/setting/`: Zones, Provinces, Regencies, Districts, Villages, Educations, Education Majors, Religions, MaritalStatuses, RelationshipTypes, Banks, EmploymentStatuses, Nationalities, JobFamilies, SalaryGrades, TER, PTKP, **Insurances**. **90 OpenAPI endpoints** (5 per entity). **Frontend:** 18 views dengan pattern Companies.vue — search bar, DataTable styling, skeleton, bilingual, dark mode. **Table:** `insurances` dibuat manual via SQL. |
