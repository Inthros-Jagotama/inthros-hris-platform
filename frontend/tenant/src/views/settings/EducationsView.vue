<template>
  <div class="space-y-1">
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('educations.edit_education') : t('educations.new_education')" modal :style="{ width: '520px' }" :closable="true" @hide="resetForm">
      <div class="space-y-4">
        <div class="space-y-2">
            <FormRow :label="t('educations.code')" required :errors="errors?.code">
            <TextInput v-model="form.code" maxlength="20" autofocus :placeholder="t('educations.code')" :class="{'p-invalid':errors?.code}" />
          </FormRow>
            <FormRow :label="t('educations.name')" required :errors="errors?.name">
            <TextInput v-model="form.name" maxlength="255" :placeholder="t('educations.name')" :class="{'p-invalid':errors?.name}" />
          </FormRow>
            <FormRow :label="t('educations.sort_order')" :errors="errors?.sort_order">
...
async function handleDelete() {
  ...
  deleteError.value = ''
  try {
    await api.delete(`/api/v1/tenant/settings/educations/${deleteTarget.value.id}`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('educations.deleted'), life: 3000 })
