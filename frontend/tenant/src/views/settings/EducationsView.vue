<template>
  <div class="space-y-1">
    <div class="flex items-center justify-between gap-2 flex-wrap">
      <div class="flex items-center gap-2">
        <IconField>
          ...
          <InputText v-model="searchQuery" :placeholder="t('educations.search')" size="small" />
        </IconField>
      </div>
      <div class="flex items-center gap-2">
        <Button :label="t('educations.new_education')" icon="pi pi-plus" size="small" @click="openDialog()" />
      </div>
    </div>
    ...
    <DataTable v-else :value="filteredItems" paginator :rows="15" size="small" class="!text-sm p-datatable-sm border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden" sortField="sort_order" :sortOrder="1">
      <template #empty>
        <div class="flex flex-col items-center justify-center py-10 text-gray-400 dark:text-gray-500">
          ...
          <p class="text-sm font-medium">{{ t('educations.empty_title') }}</p>
        </div>
      </template>
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('educations.edit_education') : t('educations.new_education')" modal :style="{ width: '520px' }" :closable="true" @hide="resetForm">
      <div class="space-y-4">
        <div>
          <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2 flex items-center gap-1.5"><i class="pi pi-graduation-cap text-indigo-400 text-sm"></i>{{ editing ? t('educations.edit_education') : t('educations.new_education') }}</h3>
          <div class="space-y-2">
            <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('educations.code') }} <span class="text-red-500">*</span></label><InputText v-model="form.code" class="!w-full" :class="{'p-invalid':errors?.code}" maxlength="20" autofocus :placeholder="t('educations.code')" /><small v-if="errors?.code" class="text-red-500 text-xs mt-1 block">{{ errors.code }}</small></div>
            <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('educations.name') }} <span class="text-red-500">*</span></label><InputText v-model="form.name" class="!w-full" :class="{'p-invalid':errors?.name}" maxlength="255" :placeholder="t('educations.name')" /><small v-if="errors?.name" class="text-red-500 text-xs mt-1 block">{{ errors.name }}</small></div>
            <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('educations.sort_order') }}</label><InputNumber v-model="form.sort_order" class="!w-full" :min="0" /></div>
          </div>
        </div>
      </div>
      <template #footer>
