<template>
  <div class="space-y-1">
    ...
    <DataTable v-else :value="items" lazy :totalRecords="totalRecords" :first="firstRecord" :rows="perPage" @page="onPage($event)" paginator paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown" :rowsPerPageOptions="[10, 15, 25, 50]" size="small" class="!text-sm p-datatable-sm border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden" sortField="sort_order" :sortOrder="1">
      ...
      <Column field="name" :header="t('banks.name')" sortable><template #body="{data}"><span class="text-navy-800 dark:text-gray-100 font-medium">{{ data.name }}</span></template></Column>
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('banks.edit') : t('banks.new')" modal :style="{ width: '520px' }" :closable="true" @hide="resetForm">
      <div class="space-y-4"><div class="space-y-2">
          <FormRow :label="t('banks.code')" required :errors="errors?.code">
            <TextInput v-model="form.code" maxlength="20" autofocus :placeholder="t('banks.code')" :class="{'p-invalid':errors?.code}" />
          </FormRow>
          <FormRow :label="t('banks.name')" required :errors="errors?.name">
            <TextInput v-model="form.name" maxlength="255" :placeholder="t('banks.name')" :class="{'p-invalid':errors?.name}" />
          </FormRow>
          <FormRow :label="t('banks.sort_order')" :errors="errors?.sort_order">
...
async function loadData() {
  loading.value = true
  try {
    const res = await api.get('/api/v1/tenant/settings/banks', {
...
async function handleDelete() {
  ...
  deleteError.value = ''
  try {
    await api.delete(`/api/v1/tenant/settings/banks/${deleteTarget.value.id}`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('banks.deleted'), life: 3000 })
