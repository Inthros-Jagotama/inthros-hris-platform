<template>
  <div class="space-y-1">
    ...
    <DataTable v-else :value="filteredItems" paginator :rows="15" size="small" class="!text-sm p-datatable-sm border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden" sortField="sort_order" :sortOrder="1">
      ...
      <Column field="description" :header="t('salary_grades.description')" style="width:160px"><template #body="{data}"><span class="text-gray-500 dark:text-gray-400 truncate block max-w-[140px]">{{ data.description || '—' }}</span></template></Column>
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('salary_grades.edit') : t('salary_grades.new')" modal :style="{ width: '560px' }" :closable="true" @hide="resetForm">
      <div class="space-y-4"><div><h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2 flex items-center gap-1.5"><i class="pi pi-chart-bar text-indigo-400 text-sm"></i>{{ editing ? t('salary_grades.edit') : t('salary_grades.new') }}</h3>
        <div class="space-y-2">
          <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('salary_grades.code') }} <span class="text-red-500">*</span></label><InputText v-model="form.code" class="!w-full" :class="{'p-invalid':errors?.code}" maxlength="20" autofocus :placeholder="t('salary_grades.code')" /><small v-if="errors?.code" class="text-red-500 text-xs mt-1 block">{{ errors.code }}</small></div>
          <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('salary_grades.name') }} <span class="text-red-500">*</span></label><InputText v-model="form.name" class="!w-full" :class="{'p-invalid':errors?.name}" maxlength="255" :placeholder="t('salary_grades.name')" /><small v-if="errors?.name" class="text-red-500 text-xs mt-1 block">{{ errors.name }}</small></div>
          <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('salary_grades.description') }}</label><textarea v-model="form.description" class="w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-emerald-500 outline-none" rows="2" :placeholder="t('salary_grades.description')"></textarea></div>
          <div class="grid grid-cols-2 gap-3">
            <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('salary_grades.min_amount') }}</label><InputNumber v-model="form.min_amount" class="!w-full" :min="0" :max="999999999999" mode="currency" currency="IDR" locale="id-ID" /></div>
            <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('salary_grades.max_amount') }}</label><InputNumber v-model="form.max_amount" class="!w-full" :min="0" :max="999999999999" mode="currency" currency="IDR" locale="id-ID" /></div>
          </div>
          <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('salary_grades.sort_order') }}</label><InputNumber v-model="form.sort_order" class="!w-full" :min="0" /></div>
        </div>
      </div></div>
...
const dialogVisible = ref(false); const editing = ref(false); const editingId = ref(null); const saving = ref(false); const errors = ref({}); const form = ref({ code: '', name: '', description: '', min_amount: 0, max_amount: 0, sort_order: 0 })
