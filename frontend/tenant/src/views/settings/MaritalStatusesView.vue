  ...
  <div class="space-y-1">
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('marital_status.edit_marital_status') : t('marital_status.new_marital_status')" modal :style="{ width: '520px' }" :closable="true" @hide="resetForm">
      <div class="space-y-4"><div class="space-y-2">
          <FormRow :label="t('marital_status.code')" required :errors="errors?.code">
            <TextInput v-model="form.code" maxlength="20" autofocus :placeholder="t('marital_status.code')" :class="{'p-invalid':errors?.code}" />
          </FormRow>
          <FormRow :label="t('marital_status.name')" required :errors="errors?.name">
            <TextInput v-model="form.name" maxlength="255" :placeholder="t('marital_status.name')" :class="{'p-invalid':errors?.name}" />
          </FormRow>
          <FormRow :label="t('marital_status.sort_order')" :errors="errors?.sort_order">
...
async function handleDelete() {
  ...
  deleteError.value = ''
  try {
    await api.delete(`/api/v1/tenant/settings/marital-statuses/${deleteTarget.value.id}`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('marital_status.deleted'), life: 3000 })
