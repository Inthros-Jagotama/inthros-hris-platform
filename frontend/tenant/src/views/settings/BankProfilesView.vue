<template>
  <div class="space-y-1">
    <div class="flex items-center justify-between gap-2 flex-wrap">
      ...
      <Button :label="t('payroll.new_bank_profile')" icon="pi pi-plus" size="small" @click="openDialog()" />
    </div>
    <DataTable
      :value="items"
      lazy
    ...
    >
      <template #empty><div class="flex flex-col items-center justify-center py-10 text-gray-400 dark:text-gray-500"><i class="pi pi-building text-3xl mb-2 opacity-50"></i><p class="text-sm font-medium">{{ t('payroll.bank_profiles_empty') }}</p></div></template>
      ...
      <Column :header="t('payroll.bank_name')" sortable style="width:120px"><template #body="{data}"><span class="text-gray-600 dark:text-gray-300 text-xs">{{ data.bank_name }}</span></template></Column>
      <Column :header="t('payroll.bank_account_number')" sortable style="width:150px"><template #body="{data}"><span class="font-mono text-gray-600 dark:text-gray-300 text-xs">{{ data.bank_account_number }}</span></template></Column>
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('payroll.edit_bank_profile') : t('payroll.new_bank_profile')" modal :style="{ width: 'min(700px, 95vw)' }" :closable="true" @hide="resetForm">
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormRow :label="t('payroll.bank_name')" required :errors="errors?.bank_name">
            <TextInput v-model="form.bank_name" maxlength="150" :placeholder="t('payroll.bank_name')" :class="{'p-invalid':errors?.bank_name}" />
          </FormRow>
          <FormRow :label="t('payroll.bank_code')">
            <TextInput v-model="form.bank_code" maxlength="50" placeholder="BRIVA / BCA / dst." />
          </FormRow>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormRow :label="t('payroll.bank_account_number')" required :errors="errors?.bank_account_number">
            <TextInput v-model="form.bank_account_number" maxlength="100" :class="{'p-invalid':errors?.bank_account_number}" />
          </FormRow>
          <FormRow :label="t('payroll.bank_account_holder_name')" required :errors="errors?.bank_account_holder_name">
            <TextInput v-model="form.bank_account_holder_name" maxlength="255" :class="{'p-invalid':errors?.bank_account_holder_name}" />
          </FormRow>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormRow :label="t('payroll.bank_branch')">
            <TextInput v-model="form.bank_branch" maxlength="150" />
          </FormRow>
    ...
    <ConfirmDeleteDialog v-model:visible="deleteDialogVisible" :title="t('payroll.bank_profiles')" :message="deleteMessage" :loading="deleting" :errorMsg="deleteError" @confirm="handleDelete" @cancel="deleteDialogVisible=false" />
  </div>
</template>
<script setup>
...
const form = ref({ employee_id: null, employee_payroll_profile_id: null, bank_code: '', bank_name: '', bank_branch: '', bank_account_number: '', bank_account_holder_name: '', is_primary: true, effective_start_date: '', effective_end_date: null, status: 'ACTIVE' })
...
const deleteMessage = computed(() => deleteTarget.value ? `${t('payroll.employee')}: ${employeeLabel(deleteTarget.value.employee_id)} — ${deleteTarget.value.bank_name} ${deleteTarget.value.bank_account_number}` : t('common.no_data'))
...
async function loadData() {
  try {
    const res = await api.get('/api/v1/tenant/payroll/employee-bank-profiles', { params: { page: currentPage.value, per_page: perPage.value } })
    const body = res.data
...
function openDialog(item) {
  ...
  form.value = {
    ...
    bank_code: item?.bank_code || '', bank_name: item?.bank_name || '', bank_branch: item?.bank_branch || '',
...
function resetForm() {
  form.value = { employee_id: null, employee_payroll_profile_id: null, bank_code: '', bank_name: '', bank_branch: '', bank_account_number: '', bank_account_holder_name: '', is_primary: true, effective_start_date: '', effective_end_date: null, status: 'ACTIVE' }
...
async function handleSave() {
  ...
  if (!form.value.bank_name?.trim()) { errors.value = { bank_name: [t('form.required')] }; return }
  if (!form.value.bank_account_number?.trim()) { errors.value = { bank_account_number: [t('form.required')] }; return }
  if (!form.value.bank_account_holder_name?.trim()) { errors.value = { bank_account_holder_name: [t('form.required')] }; return }
  ...
  try {
    const payload = {
      ...
      bank_code: form.value.bank_code || null, bank_name: form.value.bank_name.trim(), bank_branch: form.value.bank_branch || null,
      bank_account_number: form.value.bank_account_number.trim(), bank_account_holder_name: form.value.bank_account_holder_name.trim(),
      is_primary: form.value.is_primary, effective_start_date: form.value.effective_start_date,
      effective_end_date: form.value.effective_end_date || null, status: form.value.status
    }
    if (editing.value) {
      await api.put(`/api/v1/tenant/payroll/employee-bank-profiles/${editingId.value}`, payload)
...
async function handleDelete() {
  ...
  try {
    await api.delete(`/api/v1/tenant/payroll/employee-bank-profiles/${deleteTarget.value.id}`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('payroll.bank_profile_deleted'), life: 3000 })
