<template>
  <div class="space-y-1">
    ...
    <DataTable v-else :value="items" lazy :totalRecords="totalRecords" :first="firstRecord" :rows="perPage" @page="onPage($event)" paginator paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown" :rowsPerPageOptions="[10, 15, 25, 50]" size="small" class="!text-sm p-datatable-sm border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden" sortField="sort_order" :sortOrder="1">
      ...
      <Column field="name" :header="t('employment_statuses.name')" sortable><template #body="{data}"><span class="text-navy-800 dark:text-gray-100 font-medium">{{ data.name }}</span></template></Column>
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('employment_statuses.edit') : t('employment_statuses.new')" modal :style="{ width: '520px' }" :closable="true" @hide="resetForm">
      <div class="space-y-4"><div class="space-y-2">
          <FormRow :label="t('employment_statuses.code')" required :errors="errors?.code">
            <TextInput v-model="form.code" maxlength="20" autofocus :placeholder="t('employment_statuses.code')" :class="{'p-invalid':errors?.code}" />
          </FormRow>
          <FormRow :label="t('employment_statuses.name')" required :errors="errors?.name">
            <TextInput v-model="form.name" maxlength="255" :placeholder="t('employment_statuses.name')" :class="{'p-invalid':errors?.name}" />
          </FormRow>
          <FormRow :label="t('employment_statuses.sort_order')" :errors="errors?.sort_order">
...
async function handleDelete() {
  ...
  deleteError.value = ''
  try {
    await api.delete(`/api/v1/tenant/settings/employment-statuses/${deleteTarget.value.id}`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('employment_statuses.deleted'), life: 3000 })
