<template>
  <div class="space-y-1">
    <div class="flex items-center justify-between gap-2 flex-wrap">
      ...
      <Button :label="t('payroll.new_bpjs_profile')" icon="pi pi-plus" size="small" @click="openDialog()" />
    </div>
    <DataTable
      :value="items"
      lazy
    ...
    >
      <template #empty><div class="flex flex-col items-center justify-center py-10 text-gray-400 dark:text-gray-500"><i class="pi pi-shield text-3xl mb-2 opacity-50"></i><p class="text-sm font-medium">{{ t('payroll.bpjs_profiles_empty') }}</p></div></template>
      ...
      <Column :header="t('payroll.bpjs_health_no')" style="width:140px"><template #body="{data}"><span class="font-mono text-gray-600 dark:text-gray-300 text-xs">{{ data.bpjs_health_no || '-' }}</span></template></Column>
      <Column :header="t('payroll.bpjs_tk_no')" style="width:140px"><template #body="{data}"><span class="font-mono text-gray-600 dark:text-gray-300 text-xs">{{ data.bpjs_tk_no || '-' }}</span></template></Column>
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('payroll.edit_bpjs_profile') : t('payroll.new_bpjs_profile')" modal :style="{ width: 'min(700px, 95vw)' }" :closable="true" @hide="resetForm">
      <div class="space-y-4">
        ...
        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-3 flex items-center justify-between gap-3">
          ...
          <ToggleSwitch v-model="form.bpjs_health_active" class="shrink-0" />
        </div>
        <div v-if="form.bpjs_health_active" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormRow :label="t('payroll.bpjs_health_no')">
            <TextInput v-model="form.bpjs_health_no" maxlength="50" />
          </FormRow>
          <FormRow :label="t('payroll.bpjs_health_registered_name')">
            <TextInput v-model="form.bpjs_health_registered_name" maxlength="255" />
          </FormRow>
        </div>
        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-3 flex items-center justify-between gap-3">
          <div class="flex flex-col gap-0.5 min-w-0">
          ...
          <ToggleSwitch v-model="form.bpjs_tk_active" class="shrink-0" />
        </div>
        <div v-if="form.bpjs_tk_active" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormRow :label="t('payroll.bpjs_tk_no')">
            <TextInput v-model="form.bpjs_tk_no" maxlength="50" />
          </FormRow>
          <FormRow :label="t('payroll.bpjs_tk_registered_name')">
            <TextInput v-model="form.bpjs_tk_registered_name" maxlength="255" />
          </FormRow>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormRow :label="t('payroll.jkk_risk_class')">
            <SelectLabel v-model="form.jkk_risk_class" :options="riskOptions" optionLabel="label" optionValue="value" :placeholder="t('common.select')" />
          </FormRow>
          <FormRow :label="t('payroll.pension_active')">
            <ToggleSwitch v-model="form.pension_active" />
          </FormRow>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <ConfirmDeleteDialog v-model:visible="deleteDialogVisible" :title="t('payroll.bpjs_profiles')" :message="deleteMessage" :loading="deleting" :errorMsg="deleteError" @confirm="handleDelete" @cancel="deleteDialogVisible=false" />
  </div>
</template>
<script setup>
...
const form = ref({ employee_id: null, employee_payroll_profile_id: null, bpjs_health_active: false, bpjs_health_no: '', bpjs_health_registered_name: '', bpjs_tk_active: false, bpjs_tk_no: '', bpjs_tk_registered_name: '', jkk_risk_class: 'LOW', pension_active: true, effective_start_date: '', effective_end_date: null, status: 'ACTIVE', notes: '' })
...
async function loadData() {
  try {
    const res = await api.get('/api/v1/tenant/payroll/employee-bpjs-profiles', { params: { page: currentPage.value, per_page: perPage.value } })
    const body = res.data
...
function resetForm() {
  form.value = { employee_id: null, employee_payroll_profile_id: null, bpjs_health_active: false, bpjs_health_no: '', bpjs_health_registered_name: '', bpjs_tk_active: false, bpjs_tk_no: '', bpjs_tk_registered_name: '', jkk_risk_class: 'LOW', pension_active: true, effective_start_date: '', effective_end_date: null, status: 'ACTIVE', notes: '' }
...
async function handleSave() {
  ...
  try {
    const payload = {
      ...
      bpjs_health_active: form.value.bpjs_health_active, bpjs_health_no: form.value.bpjs_health_no || null, bpjs_health_registered_name: form.value.bpjs_health_registered_name || null,
      bpjs_tk_active: form.value.bpjs_tk_active, bpjs_tk_no: form.value.bpjs_tk_no || null, bpjs_tk_registered_name: form.value.bpjs_tk_registered_name || null,
      jkk_risk_class: form.value.jkk_risk_class, pension_active: form.value.pension_active,
    ...
    if (editing.value) {
      await api.put(`/api/v1/tenant/payroll/employee-bpjs-profiles/${editingId.value}`, payload)
...
async function handleDelete() {
  ...
  try {
    await api.delete(`/api/v1/tenant/payroll/employee-bpjs-profiles/${deleteTarget.value.id}`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('payroll.bpjs_profile_deleted'), life: 3000 })
