<template>
  <div class="space-y-1">
    <div class="flex items-center justify-between gap-2 flex-wrap">
      ...
      <div class="flex items-center gap-2">
        <Button :label="t('performance_formulas.new')" icon="pi pi-plus" size="small" @click="openDialog()" />
      </div>
    </div>
    ...
    <DataTable v-else :value="items" lazy :totalRecords="totalRecords" :first="firstRecord" :rows="perPage" @page="onPage($event)" paginator paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown" :rowsPerPageOptions="[10, 15, 25, 50]" size="small" class="!text-sm p-datatable-sm border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden" sortField="sort_order" :sortOrder="1">
      <template #empty>
        <div class="flex flex-col items-center justify-center py-10 text-gray-400 dark:text-gray-500">
          <i class="pi pi-calculator text-3xl mb-2 opacity-50"></i>
          <p class="text-sm font-medium">{{ t('performance_formulas.empty_title') }}</p>
        </div>
      </template>
      <Column field="code" :header="t('performance_formulas.code')" sortable style="width:120px">
        <template #body="{data}">
          <Tag :value="data.code" severity="info" class="!text-xs !px-1.5 !py-0.5" />
        </template>
      </Column>
      <Column field="name" :header="t('performance_formulas.name')" sortable>
        <template #body="{data}">
          <span class="text-gray-800 dark:text-gray-100 font-medium">{{ data.name }}</span>
        </template>
      </Column>
      <Column field="formula_type" :header="t('performance_formulas.formula_type')" sortable style="width:150px">
        <template #body="{data}">
          <Tag :value="data.formula_type" severity="secondary" class="!text-xs" />
        </template>
      </Column>
      <Column field="expression" :header="t('performance_formulas.expression')" style="width:200px">
        <template #body="{data}">
          <code v-if="data.expression" class="text-xs bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-purple-600 dark:text-purple-400">{{ data.expression }}</code>
          <span v-else class="text-gray-400">-</span>
        </template>
      </Column>
      <Column field="sort_order" :header="t('performance_formulas.sort_order')" sortable style="width:80px">
        <template #body="{data}">
    ...
    <ConfirmDeleteDialog
      ...
      :title="t('performance_formulas.delete_title')"
      :message="t('performance_formulas.delete_message', { name: deleteTarget?.name })"
      :loading="deleting"
      :error="deleteError"
...
async function loadData() {
  loading.value = true
  try {
    const res = await api.get('/api/v1/tenant/performance/indicator-formulas', {
...
function openDialog(item) {
  ...
  form.value = {
    code: item?.code || '',
    name: item?.name || '',
    formula_type: item?.formula_type || 'MANUAL',
...
async function handleSave() {
  ...
  if (!form.value.formula_type) {
    errors.value = { formula_type: [t('form.required')] }
    return
  }
  ...
  try {
    const payload = {
      ...
      formula_type: form.value.formula_type,
      expression: form.value.expression || null,
      description: form.value.description || null,
      sort_order: form.value.sort_order || 0
    }

    if (editing.value) {
      await api.put(`/api/v1/tenant/performance/indicator-formulas/${editingId.value}`, payload)
      toast.add({ severity: 'success', summary: t('message.success'), detail: t('performance_formulas.updated'), life: 3000 })
    } else {
      await api.post('/api/v1/tenant/performance/indicator-formulas', payload)
      toast.add({ severity: 'success', summary: t('message.success'), detail: t('performance_formulas.created'), life: 3000 })
    }
...
async function handleDelete() {
  ...
  deleteError.value = ''
  try {
    await api.delete(`/api/v1/tenant/performance/indicator-formulas/${deleteTarget.value.id}`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('performance_formulas.deleted'), life: 3000 })
