    ...
    <div class="flex items-center justify-between gap-2 flex-wrap">
      <div class="flex items-center gap-2">
        <IconField>
          ...
          <InputText v-model="searchQuery" :placeholder="t('provinces.search')" size="small" />
        </IconField>
      </div>
      <div class="flex items-center gap-2">
      ...
      <div class="space-y-4">
        <div>
          <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2 flex items-center gap-1.5">
            <i class="pi pi-globe text-indigo-400 text-sm"></i>
            {{ editing ? t('provinces.edit_province') : t('provinces.new_province') }}
          </h3>
          <div class="space-y-2">
            <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('provinces.code') }} <span class="text-red-500">*</span></label><InputText v-model="form.code" class="!w-full" :class="{'p-invalid':errors?.code}" maxlength="10" autofocus :placeholder="t('provinces.code')" /><small v-if="errors?.code" class="text-red-500 text-xs mt-1 block">{{ errors.code }}</small></div>
            <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('provinces.name') }} <span class="text-red-500">*</span></label><InputText v-model="form.name" class="!w-full" :class="{'p-invalid':errors?.name}" maxlength="255" :placeholder="t('provinces.name')" /><small v-if="errors?.name" class="text-red-500 text-xs mt-1 block">{{ errors.name }}</small></div>
          </div>
        </div>
      </div>
      <template #footer>
...
async function loadData() {
  loading.value = true
  try {
    const res = await api.get('/api/v1/tenant/settings/provinces?per_page=200')
...
function confirmDelete(item) {
  confirm.require({
    header: t('provinces.confirm_delete_title'),
    message: t('provinces.confirm_delete', { name: item.name }),
    ...
    accept: async () => {
      try {
        await api.delete(`/api/v1/tenant/settings/provinces/${item.id}`)
