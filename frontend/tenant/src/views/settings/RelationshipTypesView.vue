  ...
  <div class="space-y-1">
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('relationship_types.edit') : t('relationship_types.new')" modal :style="{ width: '520px' }" :closable="true" @hide="resetForm">
      <div class="space-y-4"><div class="space-y-2">
          <FormRow :label="t('relationship_types.code')" required :errors="errors?.code">
            <TextInput v-model="form.code" maxlength="20" autofocus :placeholder="t('relationship_types.code')" :class="{'p-invalid':errors?.code}" />
          </FormRow>
          <FormRow :label="t('relationship_types.name')" required :errors="errors?.name">
            <TextInput v-model="form.name" maxlength="255" :placeholder="t('relationship_types.name')" :class="{'p-invalid':errors?.name}" />
          </FormRow>
          <FormRow :label="t('relationship_types.sort_order')" :errors="errors?.sort_order">
...
async function handleDelete() {
  ...
  deleteError.value = ''
  try {
    await api.delete(`/api/v1/tenant/settings/relationship-types/${deleteTarget.value.id}`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('relationship_types.deleted'), life: 3000 })
