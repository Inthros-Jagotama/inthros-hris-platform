<template>
  <div class="space-y-1">
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('ters.edit') : t('ters.new')" modal :style="{ width: '520px' }" :closable="true" @hide="resetForm">
      <div class="space-y-4"><div><h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2 flex items-center gap-1.5"><i class="pi pi-calculator text-indigo-400 text-sm"></i>{{ editing ? t('ters.edit') : t('ters.new') }}</h3>
        <div class="space-y-2">
          <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('ters.group') }} <span class="text-red-500">*</span></label>
            <Select v-model="form.group" :options="groupOptions" optionValue="value" optionLabel="label" :placeholder="t('ters.select_group')" class="!w-full" :class="{'p-invalid':errors?.group}" :showClear="true" />
            <small v-if="errors?.group" class="text-red-500 text-xs mt-1 block">{{ errors.group }}</small></div>
          <div class="grid grid-cols-2 gap-3">
            <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('ters.bruto_min') }}</label><InputNumber v-model="form.bruto_min" class="!w-full" :min="0" mode="currency" currency="IDR" locale="id-ID" /></div>
            <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('ters.bruto_max') }}</label><InputNumber v-model="form.bruto_max" class="!w-full" :min="0" mode="currency" currency="IDR" locale="id-ID" /></div>
          </div>
          <div><label class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ t('ters.rate') }} (%) <span class="text-red-500">*</span></label><InputNumber v-model="form.rate" class="!w-full" :min="0" :max="100" :step="0.01" :minFractionDigits="2" :maxFractionDigits="2" /><small v-if="errors?.rate" class="text-red-500 text-xs mt-1 block">{{ errors.rate }}</small></div>
        </div>
      </div></div>
...
const dialogVisible = ref(false); const editing = ref(false); const editingId = ref(null); const saving = ref(false); const errors = ref({}); const form = ref({ group: '', bruto_min: null, bruto_max: null, rate: 0 })
...
function confirmDelete(item) { confirm.require({header:t('ters.confirm_delete_title'),message:t('ters.confirm_delete',{name:`${item.group} ${item.rate}%`}),icon:'pi pi-exclamation-triangle',rejectLabel:t('common.cancel'),acceptLabel:t('common.delete'),rejectClass:'p-button-outlined p-button-secondary',acceptClass:'p-button-danger',accept:async()=>{try{await api.delete(`/api/v1/tenant/settings/ters/${item.id}`);toast.add({severity:'success',summary:t('message.success'),detail:t('ters.deleted'),life:3000});await loadData()}catch(e){toast.add({severity:'error',summary:t('message.error'),detail:e.response?.data?.error?.message||t('message.operation_failed'),life:4000})}}}) }
onMounted(loadData)
</script>
