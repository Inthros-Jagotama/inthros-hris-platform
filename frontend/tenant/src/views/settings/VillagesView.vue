<template>
  <div class="space-y-1">
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? t('villages.edit_village') : t('villages.new_village')" modal :style="{ width: '520px' }" :closable="true" @hide="resetForm">
      <div class="space-y-4">
        <div class="space-y-2">
            <FormRow :label="t('villages.code')" required :errors="errors?.code">
            <TextInput v-model="form.code" maxlength="15" autofocus :placeholder="t('villages.code')" :class="{'p-invalid':errors?.code}" />
          </FormRow>
            <FormRow :label="t('villages.name')" required :errors="errors?.name">
            <TextInput v-model="form.name" maxlength="255" :placeholder="t('villages.name')" :class="{'p-invalid':errors?.name}" />
          </FormRow>
...
async function handleDelete() {
  ...
  deleteError.value = ''
  try {
    await api.delete(`/api/v1/tenant/settings/villages/${deleteTarget.value.id}`)
