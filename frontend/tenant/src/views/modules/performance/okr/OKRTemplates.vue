<template>
  <div class="space-y-1">
    ...
    <div class="flex items-center justify-between gap-2 flex-wrap mb-2">
      ...
      <div class="flex items-center gap-2">
        <div class="relative w-64 max-w-full">
          ...
          <InputText
            v-model="searchQuery"
            :placeholder="t('okr.search_templates')"
        ...
        <Button :label="t('okr.new_template')" icon="pi pi-plus" size="small" @click="createTemplate" />
      </div>
    </div>
    ...
    >
      <template #empty>
        <div class="flex flex-col items-center justify-center py-10 text-gray-400 dark:text-gray-500">
          ...
          <p class="text-sm font-medium">{{ t('okr.empty_templates') }}</p>
          <p class="text-xs mt-1">{{ t('okr.empty_templates_hint') }}</p>
        </div>
      </template>
      ...
      <Column field="organization_name" :header="t('okr.organization')" sortable style="width:200px">
        <template #body="{data}">
          <span class="text-gray-600 dark:text-gray-300">{{ data.organization_name || '-' }}</span>
        </template>
      </Column>

      <Column field="period_code" :header="t('okr.period')" sortable style="width:120px">
        <template #body="{data}">
      ...
      <Column :header="t('okr.objectives')" style="width:100px">
        <template #body="{data}">
          <span class="text-gray-600 dark:text-gray-300 font-mono">{{ data.objective_count || 0 }}</span>
        </template>
      </Column>
      ...
      <Column :header="t('common.actions')" style="width:120px" frozen alignFrozen="right">
        <template #body="{data}">
          <div v-if="canManageTemplate(data)" class="flex items-center gap-1">
            ...
            <Button icon="pi pi-copy" size="small" text severity="info" v-tooltip.left="t('okr.duplicate')" @click="duplicateTemplate(data)" />
            <Button icon="pi pi-trash" size="small" text severity="danger" v-tooltip.left="t('common.delete')" @click="confirmDelete(data)" />
          </div>
          <span v-else v-tooltip.left="t('okr.cannot_manage_template')" class="inline-flex items-center justify-center w-7 h-7 text-gray-300 dark:text-gray-600">
    ...
    <ConfirmDeleteDialog
      ...
      :title="t('okr.delete_template_title')"
      :message="t('okr.delete_template_message', { name: deleteTarget?.name })"
      :loading="deleting"
      :error="deleteError"
...
async function loadData() {
  loading.value = true
  try {
    const res = await api.get('/api/v1/tenant/performance/okr/templates', {
      params: {
...
function createTemplate() {
  router.push('/performance/okr/templates/new')
}

function editTemplate(item) {
  router.push(`/performance/okr/templates/${item.id}/edit`)
}

async function duplicateTemplate(item) {
  try {
    const res = await api.post(`/api/v1/tenant/performance/okr/templates/${item.id}/duplicate`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('okr.template_duplicated'), life: 3000 })
    const newId = res.data?.data?.id || res.data?.id
    if (newId) {
      router.push(`/performance/okr/templates/${newId}/edit`)
    } else {
      await loadData()
    }
  } catch (e) {
...
async function handleDelete() {
  ...
  deleteError.value = ''
  try {
    await api.delete(`/api/v1/tenant/performance/okr/templates/${deleteTarget.value.id}`)
    toast.add({ severity: 'success', summary: t('message.success'), detail: t('okr.template_deleted'), life: 3000 })
