<template>
  <div class="space-y-4 max-w-2xl">
    ...
    <div v-else-if="!context.has_position" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-8 text-center">
      ...
      <p class="text-sm font-semibold text-gray-700 dark:text-gray-200">{{ t('kpi.no_position_title') }}</p>
      <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ t('kpi.no_position_hint') }}</p>
    </div>

    <!-- No published template for this position -->
    <div v-else-if="context.templates.length === 0" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-8 text-center">
      ...
      <p class="text-sm font-semibold text-gray-700 dark:text-gray-200">{{ t('kpi.no_template_title') }}</p>
      <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ t('kpi.no_template_hint', { organization: context.organization_name }) }}</p>
    </div>
    ...
    <div v-else class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 space-y-4">
      ...
      <FormRow :label="t('kpi.period')" required>
        <Select v-model="selectedPeriodId" :options="periodOptions" optionLabel="label" optionValue="value" :placeholder="t('kpi.select_period')" class="w-full" @change="checkExistingEvaluation" />
      </FormRow>
      ...
      <div v-for="tpl in context.templates" :key="tpl.id" class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 flex items-center justify-between gap-3">
        ...
        <Button
          :label="existingEvaluationId ? t('kpi.continue_self_assessment') : t('kpi.start_self_assessment')"
      ...
      <p v-if="existingEvaluationId" class="text-xs text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
        <i class="pi pi-check-circle"></i> {{ t('kpi.existing_evaluation_found') }}
      </p>
    </div>
  </div>
</template>
...
async function checkExistingEvaluation() {
  ...
  try {
    const res = await api.get('/api/v1/tenant/performance/kpi/evaluations', {
...
async function startAssessment(template) {
  if (!selectedPeriodId.value) {
    toast.add({ severity: 'warn', summary: t('message.error'), detail: t('kpi.period_required'), life: 3000 })
    return
  }

  if (existingEvaluationId.value) {
    router.push(`/performance/kpi/evaluation/${existingEvaluationId.value}`)
    return
  }
