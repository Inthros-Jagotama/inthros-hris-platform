  ...
  <div class="space-y-1">
    <div class="flex items-center justify-between gap-2 flex-wrap">
      ...
      <div class="flex items-center gap-2 ml-auto">
        <Button :label="t('common.back')" icon="pi pi-arrow-left" size="small" severity="secondary" outlined @click="router.push('/reimbursements')" />
        <Button v-if="canManage" :label="t('reimbursement.types_new')" icon="pi pi-plus" size="small" @click="openDialog()" />
      </div>
    </div>
    ...
    >
      <template #empty>
        <div class="flex flex-col items-center justify-center py-10 text-gray-400 dark:text-gray-500">
          ...
          <p class="text-sm font-medium">{{ t('reimbursement.types_empty') }}</p>
        </div>
      </template>
      <Column field="code" :header="t('reimbursement.code')" style="width:140px">
      ...
      <Column field="is_active" :header="t('reimbursement.is_active')" style="width:100px">
      ...
      <Column :header="t('common.actions')" style="width:100px" frozen alignFrozen="right">
        <template #body="{data}">
          <div class="flex items-center gap-1 justify-end">
            <Button v-if="canManage" icon="pi pi-pencil" size="small" text severity="secondary" v-tooltip.left="t('common.edit')" @click="openDialog(data)" />
            <Button v-if="canManage" icon="pi pi-trash" size="small" text severity="danger" v-tooltip.left="t('common.delete')" @click="confirmDelete(data)" />
          </div>
        </template>
      </Column>
    </DataTable>

    <Dialog v-model:visible="dialogVisible" :header="editing ? t('reimbursement.types_edit') : t('reimbursement.types_new')" modal :style="{ width: '520px' }" @hide="resetForm">
      <div class="space-y-3">
        ...
        <FormRow :label="t('reimbursement.is_active')">
    ...
    <ConfirmDeleteDialog
      ...
      :title="t('reimbursement.confirm_delete_type_title')"
      :message="t('reimbursement.confirm_delete_type', { name: deleteTarget?.name || '' })"
      :loading="deleting"
...
const canManage = computed(() => hasPermission('reimbursement.create') && hasPermission('reimbursement.update') && hasPermission('reimbursement.delete'))
...
async function loadData() {
  ...
  try {
    ...
    const res = await api.get('/api/v1/tenant/reimbursements/types', { params })
    const body = res.data
...
async function handleSave() {
  ...
  saving.value = true
  try {
    // code tidak dikirim — backend auto-generate dari name (UPPER_SNAKE_CASE)
    const payload = {
    ...
    if (editing.value) {
      await api.put(`/api/v1/tenant/reimbursements/types/${editingId.value}`, payload)
    } else {
      await api.post('/api/v1/tenant/reimbursements/types', payload)
    }
...
async function handleDelete() {
  ...
  deleteError.value = ''
  try {
    await api.delete(`/api/v1/tenant/reimbursements/types/${deleteTarget.value.id}`)
