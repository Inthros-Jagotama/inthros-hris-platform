<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <div><h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100">{{ t('job_management.subordinate_controls') }}</h2><p class="text-sm text-gray-500 dark:text-gray-400">{{ t('job_management.subordinate_description') }}</p></div>
      <Button :label="t('common.create')" icon="pi pi-plus" size="small" @click="openCreate()" />
    </div>
    <DataTableSection :items="items" :loading="loading" :total="total" :columns="cols" entity="subordinate-controls" :org-id="orgId" :on-load="loadData" @edit="openEdit" @delete="confirmDelete">
      <template #empty><div class="flex flex-col items-center justify-center py-10 text-gray-400"><i class="pi pi-sitemap text-3xl mb-2 opacity-50"></i><p class="text-sm font-medium">{{ t('job_management.empty_subordinates') }}</p></div></template>
    </DataTableSection>
    <DialogForm v-model:visible="dialogVisible" :title="editing ? t('common.edit') : t('common.create')" :saving="saving" :errors="errors" @save="handleSave" @cancel="dialogVisible=false">
      ...
      <FormRow :label="t('job_management.control_type')" :errors="errors?.job_management_value_id"><SelectLabel v-model="form.job_management_value_id" :options="allOptions" optionLabel="label" optionValue="value" :placeholder="t('common.select')" showClear /></FormRow>
    </DialogForm>
...
const { t } = useI18n(); const toast = useToast(); const apiBase = '/api/v1/tenant/job-management/subordinate-controls'
...
const form=ref({ nomenclature:'', full_code:'', job_management_value_id:'' })
...
function openCreate(){ editing.value=false; editId.value=''; form.value={nomenclature:'',full_code:'',job_management_value_id:''}; errors.value={}; dialogVisible.value=true }
function openEdit(d){ editing.value=true; editId.value=d.id; form.value={nomenclature:d.nomenclature||'',full_code:d.full_code||'',job_management_value_id:d.job_management_value_id||''}; errors.value={}; dialogVisible.value=true }
