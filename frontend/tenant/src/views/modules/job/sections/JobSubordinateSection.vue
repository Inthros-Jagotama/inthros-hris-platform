<template>
  <div class="space-y-4">
    <div>
      <h2 class="text-lg font-semibold text-navy-800 dark:text-gray-100">{{ t('job_management.subordinate_controls') }}</h2>
      <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('job_management.subordinate_description') }}</p>
    </div>

    <div class="max-w-2xl">
      <!-- Skeleton while loading subordinate data -->
      ...
      <div v-else class="space-y-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
        ...
        <FormRow :label="t('job_management.control_type')" :errors="errors?.job_management_value_id">
          <SelectLabel
...
const apiBase = '/api/v1/tenant/job-management/subordinate-controls'

const controlOptions = ref([])

// Muat opsi jenis bawahan dari job_management_values (type=subordinate)
async function loadOptions() {
  try {
    const res = await api.get('/api/v1/tenant/job-management/values', { params: { type: 'subordinate', per_page: 100 } })
    // Label menampilkan level (mis. 'Lv.1 — ...') untuk memudahkan identifikasi
    controlOptions.value = (res.data?.data || []).map(v => ({ label: `Lv.${v.level} — ${v.descriptions}`, value: v.id }))
  } catch { /* ignore */ }
}
