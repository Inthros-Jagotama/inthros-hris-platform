<template>
  <div class="space-y-4">
    ...
    <DataTableSection :items="items" :loading="loading" :total="total" :columns="cols" entity="operational-authorities" :org-id="orgId" :on-load="loadData" @edit="openEdit" @delete="confirmDelete">
      <template #empty><div class="flex flex-col items-center justify-center py-10 text-gray-400"><i class="pi pi-cog text-3xl mb-2 opacity-50"></i><p class="text-sm font-medium">{{ t('job_management.empty_authorities') }}</p></div></template>
    </DataTableSection>
    <DialogForm v-model:visible="dialogVisible" :title="dialogTitle" :saving="saving" :errors="errors" @save="handleSave" @cancel="dialogVisible=false">
      <FormRow :label="t('job_management.description')" :errors="errors?.description"><Textarea v-model="form.description" class="w-full" rows="3" :class="{'p-invalid':errors?.description}" /></FormRow>
    </DialogForm>
...
const apiBase = '/api/v1/tenant/job-management/operational-authorities'
...
const dialogTitle = computed(() => {
  const base = t('job_management.op_authorities')
