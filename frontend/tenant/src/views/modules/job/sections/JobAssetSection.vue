<template>
  <div class="space-y-4">
    <div>
      <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100">{{ t('job_management.assets') }}</h2>
...
async function loadOptions() {
  try {
    ...
    assetOptions.value = (assetRes.data?.data || []).map(v => ({ label: v.descriptions, value: v.id }))
